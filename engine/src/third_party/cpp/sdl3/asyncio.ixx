...
namespace SDL
{
    ...
    struct AsyncIO : AsyncIOBase
    {
         ...
         */
        constexpr explicit AsyncIO(AsyncIORaw resource) noexcept : AsyncIOBase(resource)
        {
        }

        /// Move constructor
        constexpr AsyncIO(AsyncIO &&other) noexcept : AsyncIO(other.release())
        {
        }
    ...
    constexpr AsyncIOTaskType ASYNCIO_TASK_READ = SDL_ASYNCIO_TASK_READ; ///< A read operation.

    constexpr AsyncIOTaskType ASYNCIO_TASK_WRITE = SDL_ASYNCIO_TASK_WRITE; ///< A write operation.

    constexpr AsyncIOTaskType ASYNCIO_TASK_CLOSE = SDL_ASYNCIO_TASK_CLOSE; ///< A close operation.
    ...
    constexpr AsyncIOResult ASYNCIO_COMPLETE = SDL_ASYNCIO_COMPLETE; ///< request was completed without error

    constexpr AsyncIOResult ASYNCIO_FAILURE =
        SDL_ASYNCIO_FAILURE; ///< request failed for some reason; check GetError()!

    constexpr AsyncIOResult ASYNCIO_CANCELED = SDL_ASYNCIO_CANCELED; ///< request was canceled before completing.
    ...
    struct AsyncIOQueue : AsyncIOQueueBase
    {
         ...
         * This assumes the ownership, call release() if you need to take back.
         */
        constexpr explicit AsyncIOQueue(AsyncIOQueueRaw resource) noexcept : AsyncIOQueueBase(resource)
        {
        }

        /// Move constructor
        constexpr AsyncIOQueue(AsyncIOQueue &&other) noexcept : AsyncIOQueue(other.release())
        {
        }
    ...
    inline AsyncIO::AsyncIO(StringParam file, StringParam mode) : AsyncIO(SDL_AsyncIOFromFile(file, mode))
    {
    }
    ...
    inline Sint64 AsyncIOBase::GetSize()
    {
        return SDL::GetAsyncIOSize(get());
    }
    ...
    inline void AsyncIOBase::Read(void *ptr, Uint64 offset, Uint64 size, AsyncIOQueueRef queue, void *userdata)
    {
    ...
    inline void AsyncIOBase::Write(void *ptr, Uint64 offset, Uint64 size, AsyncIOQueueRef queue, void *userdata)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void CloseAsyncIO(AsyncIORaw asyncio, bool flush, SDL_AsyncIOQueue *queue, void *userdata)
    {
        CheckError(SDL_CloseAsyncIO(asyncio, flush, queue, userdata));
    }

    inline bool AsyncIOBase::Close(bool flush, SDL_AsyncIOQueue *queue, void *userdata)
    {
     ...
     * @sa WaitAsyncIOResult
     */
    inline AsyncIOQueue CreateAsyncIOQueue()
    {
        return AsyncIOQueue();
    }

    inline AsyncIOQueue::AsyncIOQueue() : AsyncIOQueue(SDL_CreateAsyncIOQueue())
    {
    }
    ...
    inline void AsyncIOQueueBase::Destroy()
    {
        DestroyAsyncIOQueue(release());
    }
     ...
     * @sa SignalAsyncIOQueue
     */
    inline std::optional<AsyncIOOutcome> WaitAsyncIOResult(AsyncIOQueueRef queue, Milliseconds timeout)
    {
    ...
    inline std::optional<AsyncIOOutcome> AsyncIOQueueBase::WaitResult(Milliseconds timeout)
    {
    ...
    inline void AsyncIOQueueBase::Signal()
    {
        SDL::SignalAsyncIOQueue(get());
    }
