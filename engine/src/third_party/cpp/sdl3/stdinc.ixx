...
#define SDL_UINT64_C(c) c##ULL /* or whatever the current compiler uses. */
    ...
    class Time
    {
      ...
      public:
         ...
         */
        constexpr Time(TimeRaw time = {}) noexcept : m_time(time)
        {
        }
         ...
         */
        constexpr operator TimeRaw() const noexcept
        {
            return m_time;
        }
    ...
    inline void SetMemoryFunctions(malloc_func malloc_func,
                                   ...
                                   free_func free_func)
    {
        CheckError(SDL_SetMemoryFunctions(malloc_func, calloc_func, realloc_func, free_func));
    }
    ...
    struct Environment : EnvironmentBase
    {
         ...
         */
        constexpr explicit Environment(EnvironmentRaw resource) noexcept : EnvironmentBase(resource)
        {
        }

        /// Move constructor
        constexpr Environment(Environment &&other) noexcept : Environment(other.release())
        {
        }
     ...
     * @sa UnsetEnvironmentVariable
     */
    inline EnvironmentRaw GetEnvironment()
    {
    ...
    inline Environment::Environment(bool populated) : Environment(SDL_CreateEnvironment(populated))
    {
    }
     ...
     * @sa UnsetEnvironmentVariable
     */
    inline void SetEnvironmentVariable(EnvironmentRef env, StringParam name, StringParam value, bool overwrite)
    {
        CheckError(SDL_SetEnvironmentVariable(env, name, value, overwrite));
    }

    inline void EnvironmentBase::SetVariable(StringParam name, StringParam value, bool overwrite)
    {
        SDL::SetEnvironmentVariable(get(), std::move(name), std::move(value), overwrite);
    }
    ...
    inline void EnvironmentBase::Destroy()
    {
        DestroyEnvironment(release());
    }
     ...
     * data key = { 2, nullptr };
     *
     * data *result = bsearch_r(&key, values, arraysize(values), sizeof(values[0]), compare, (const void
     ...
     * @sa qsort_r
     */
    inline void *bsearch_r(const void *key, const void *base, size_t nmemb, size_t size, CompareCB compare)
    {
        static_assert(false, "Not implemented");
    }
    ...
    class Random
    {
      ...
      public:
        constexpr Random() : m_state(0)
        {
        }

        constexpr explicit Random(Uint64 state) : m_state(state)
        {
        }
        ...
        Uint32 rand_bits()
        {
            return SDL_rand_bits_r(&m_state);
        }
    };
     ...
     * @sa randf
     */
    inline Sint32 rand(Uint64 *state, Sint32 n)
    {
     ...
     * @sa randf
     */
    inline Sint32 rand(Random &state, Sint32 n)
    {
        return state.rand(n);
    }
     ...
     * @sa randf
     */
    inline float randf(Uint64 *state)
    {
     ...
     * @sa rand
     */
    inline float randf(Random &state)
    {
        return state.randf();
    }
     ...
     * @sa randf
     */
    inline Uint32 rand_bits(Uint64 *state)
    {
     ...
     * @sa srand
     */
    inline Uint32 rand_bits(Random &state)
    {
        return state.rand_bits();
    }
    ...
    struct IConv : IConvBase
    {
         ...
         * This assumes the ownership, call release() if you need to take back.
         */
        constexpr explicit IConv(IConvRaw resource) noexcept : IConvBase(resource)
        {
        }

        /// Move constructor
        constexpr IConv(IConv &&other) noexcept : IConv(other.release())
        {
        }
         ...
         * @post a handle that must be freed with iconv_close, or ICONV_ERROR on
         *       failure.
         *
         * @threadsafety It is safe to call this function from any thread.
         *
    ...
    inline IConv::IConv(StringParam tocode, StringParam fromcode) : IConv(SDL_iconv_open(tocode, fromcode))
    {
    }
     ...
     * @sa iconv_string
     */
    inline int iconv_close(IConvRaw cd)
    {
        return SDL_iconv_close(cd);
    }

    inline int IConvBase::close()
    {
        return iconv_close(release());
    }
     ...
     * @sa iconv_string
     */
    inline size_t iconv(IConv cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft)
    {
        return SDL_iconv(cd, inbuf, inbytesleft, outbuf, outbytesleft);
    }

    inline size_t IConvBase::iconv(const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft)
    {
        return SDL::iconv(get(), inbuf, inbytesleft, outbuf, outbytesleft);
    }

    constexpr size_t ICONV_ERROR = SDL_ICONV_ERROR; ///< Generic error. Check GetError()?

    constexpr size_t ICONV_E2BIG = SDL_ICONV_E2BIG; ///< Output buffer was too small.

    constexpr size_t ICONV_EILSEQ = SDL_ICONV_EILSEQ; ///< Invalid input sequence was encountered.

    constexpr size_t ICONV_EINVAL = SDL_ICONV_EINVAL; ///< Incomplete input sequence was encountered.
     ...
     * @sa iconv
     */
    inline OwnArray<char> iconv_string(StringParam tocode, StringParam fromcode, SourceBytes inbuf)
    {
