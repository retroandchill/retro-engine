...
namespace SDL
{
    ...
    constexpr SystemCursor SYSTEM_CURSOR_DEFAULT = SDL_SYSTEM_CURSOR_DEFAULT; ///< Default cursor. Usually an arrow.

    constexpr SystemCursor SYSTEM_CURSOR_TEXT = SDL_SYSTEM_CURSOR_TEXT; ///< Text selection. Usually an I-beam.

    /// Wait. Usually an hourglass or watch or spinning ball.
    constexpr SystemCursor SYSTEM_CURSOR_WAIT = SDL_SYSTEM_CURSOR_WAIT;

    constexpr SystemCursor SYSTEM_CURSOR_CROSSHAIR = SDL_SYSTEM_CURSOR_CROSSHAIR; ///< Crosshair.
    ...
    constexpr SystemCursor SYSTEM_CURSOR_NOT_ALLOWED = SDL_SYSTEM_CURSOR_NOT_ALLOWED;
    ...
    struct Cursor : CursorBase
    {
         ...
         */
        constexpr explicit Cursor(CursorRaw resource) noexcept : CursorBase(resource)
        {
        }

        /// Move constructor
        constexpr Cursor(Cursor &&other) noexcept : Cursor(other.release())
        {
        }
    ...
    constexpr MouseWheelDirection MOUSEWHEEL_NORMAL = SDL_MOUSEWHEEL_NORMAL; ///< The scroll direction is normal
    ...
    using MouseMotionTransformCallback =
        void(SDLCALL *)(void *userdata, Uint64 timestamp, WindowRaw window, MouseID mouseID, float *x, float *y);
    ...
    using MouseMotionTransformCB =
        MakeFrontCallback<void(Uint64 timestamp, WindowRaw window, MouseID mouseID, float *x, float *y)>;
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline WindowRef GetMouseFocus()
    {
    ...
    {
        SDL_WarpMouseInWindow(window, p);
    }
     ...
     */
    inline void SetRelativeMouseTransform(MouseMotionTransformCallback callback, void *userdata)
    {
     ...
     * @sa SetCursor
     */
    inline Cursor CreateCursor(const Uint8 *data, const Uint8 *mask, const PointRaw &size, const PointRaw &hot)
    {
        return Cursor(data, mask, size, hot);
    }

    inline Cursor::Cursor(const Uint8 *data, const Uint8 *mask, const PointRaw &size, const PointRaw &hot)
        : Cursor(CheckError(SDL_CreateCursor(data, mask, size, hot)))
    {
    }

    inline Cursor::Cursor(SurfaceRef surface, const PointRaw &hot)
        : Cursor(CheckError(SDL_CreateColorCursor(surface, hot)))
    {
    }

    inline Cursor::Cursor(SystemCursor id) : Cursor(CheckError(SDL_CreateSystemCursor(id)))
    {
    }
     ...
     * @sa SetCursor
     */
    inline CursorRef CreateAnimatedCursor(CursorFrameInfo *frames, int frame_count, int hot_x, int hot_y)
    {
        return CheckError(SDL_CreateAnimatedCursor(frames, frame_count, hot_x, hot_y));
    }
     ...
     * @sa DestroyCursor
     */
    inline Cursor CreateSystemCursor(SystemCursor id)
    {
        return Cursor(id);
    }
    ...
    {
        CheckError(SDL_SetCursor(cursor));
    }

    inline void CursorBase::Set()
    {
        SDL::SetCursor(get());
    }
     ...
     * @sa CreateSystemCursor
     */
    inline void DestroyCursor(CursorRaw cursor)
    {
        SDL_DestroyCursor(cursor);
    }

    inline void CursorBase::Destroy()
    {
        DestroyCursor(release());
    }
