...
namespace SDL
{
    ...
    /**
     * @defgroup CategoryThread Category Thread
     *
    ...
    constexpr ThreadPriority THREAD_PRIORITY_NORMAL = SDL_THREAD_PRIORITY_NORMAL; ///< NORMAL

    constexpr ThreadPriority THREAD_PRIORITY_HIGH = SDL_THREAD_PRIORITY_HIGH; ///< HIGH

    constexpr ThreadPriority THREAD_PRIORITY_TIME_CRITICAL = SDL_THREAD_PRIORITY_TIME_CRITICAL; ///< TIME_CRITICAL
    ...
    constexpr ThreadState THREAD_UNKNOWN = SDL_THREAD_UNKNOWN; ///< The thread is not valid

    constexpr ThreadState THREAD_ALIVE = SDL_THREAD_ALIVE; ///< The thread is currently running

    constexpr ThreadState THREAD_DETACHED = SDL_THREAD_DETACHED; ///< The thread is detached and can't be waited on
    ...
    struct Thread : ThreadBase
    {
         ...
         */
        constexpr explicit Thread(ThreadRaw resource) noexcept : ThreadBase(resource)
        {
        }

        /// Move constructor
        constexpr Thread(Thread &&other) noexcept : Thread(other.release())
        {
        }
    ...
    inline Thread::Thread(PropertiesRef props) : Thread(CheckError(SDL_CreateThreadWithProperties(props)))
    {
    }
    ...
    namespace prop::Thread::Create
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_THREAD_CREATE_NAME_STRING; ///< String for name.

        constexpr auto USERDATA_POINTER = SDL_PROP_THREAD_CREATE_USERDATA_POINTER; ///< Pointer to userdata.

        constexpr auto STACKSIZE_NUMBER = SDL_PROP_THREAD_CREATE_STACKSIZE_NUMBER; ///< Number for stacksize.
    ...
    inline ThreadID ThreadBase::GetID() const
    {
        return SDL::GetThreadID(get());
    }
    ...
    inline void ThreadBase::Wait(int *status)
    {
        SDL::WaitThread(get(), status);
    }
     ...
     * @sa WaitThread
     */
    inline void DetachThread(ThreadRaw thread)
    {
     ...
     * @sa GetTLS
     */
    inline void SetTLS(TLSID *id, const void *value, TLSDestructorCallback destructor)
    {
