...
namespace SDL
{
    ...
    constexpr SensorType SENSOR_INVALID = SDL_SENSOR_INVALID; ///< Returned for an invalid sensor

    constexpr SensorType SENSOR_UNKNOWN = SDL_SENSOR_UNKNOWN; ///< Unknown sensor type
    ...
    constexpr SensorType SENSOR_GYRO_L = SDL_SENSOR_GYRO_L; ///< Gyroscope for left Joy-Con controller

    constexpr SensorType SENSOR_ACCEL_R = SDL_SENSOR_ACCEL_R; ///< Accelerometer for right Joy-Con controller

    constexpr SensorType SENSOR_GYRO_R = SDL_SENSOR_GYRO_R; ///< Gyroscope for right Joy-Con controller
    ...
    struct Sensor : SensorBase
    {
         ...
         */
        constexpr explicit Sensor(SensorRaw resource) noexcept : SensorBase(resource)
        {
        }

        /// Move constructor
        constexpr Sensor(Sensor &&other) noexcept : Sensor(other.release())
        {
        }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline Sensor OpenSensor(SensorID instance_id)
    {
        return Sensor(instance_id);
    }

    inline Sensor::Sensor(SensorID instance_id) : Sensor(SDL_OpenSensor(instance_id))
    {
    }
    ...
    inline const char *SensorBase::GetName()
    {
        return SDL::GetSensorName(get());
    }
    ...
    inline SensorType SensorBase::GetType()
    {
        return SDL::GetSensorType(get());
    }
    ...
    inline SensorID SensorBase::GetID()
    {
        return SDL::GetSensorID(get());
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void CloseSensor(SensorRaw sensor)
    {
        SDL_CloseSensor(sensor);
    }

    inline void SensorBase::Close()
    {
        CloseSensor(release());
    }
