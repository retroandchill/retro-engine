...
namespace SDL
{
    ...
    constexpr SurfaceFlags SURFACE_PREALLOCATED = SDL_SURFACE_PREALLOCATED; ///< Surface uses preallocated pixel memory

    constexpr SurfaceFlags SURFACE_LOCK_NEEDED =
        SDL_SURFACE_LOCK_NEEDED; ///< Surface needs to be locked to access pixels

    constexpr SurfaceFlags SURFACE_LOCKED = SDL_SURFACE_LOCKED; ///< Surface is currently locked
     ...
     */
    constexpr bool MustLock(SurfaceConstRef S) const
    {
        return SDL_MUSTLOCK(S);
    }
    ...
    constexpr ScaleMode SCALEMODE_NEAREST = SDL_SCALEMODE_NEAREST; ///< nearest pixel sampling

    constexpr ScaleMode SCALEMODE_LINEAR = SDL_SCALEMODE_LINEAR; ///< linear filtering
    ...
    constexpr FlipMode FLIP_HORIZONTAL_AND_VERTICAL = SDL_FLIP_HORIZONTAL_AND_VERTICAL;
    ...
    struct SurfaceBase : ResourceBaseT<SurfaceRaw, SurfaceRawConst>
    {
         ...
         * Once you are done accessing the surface, you should use UnlockSurface() to
         * release it.
         *
         * Not all surfaces require locking. If `MustLock(surface)` evaluates to 0,
         ...
         * @param closeio if true, calls CloseIO() on `dst` before returning, even in
    ...
    struct Surface : SurfaceBase
    {
         ...
         */
        constexpr explicit Surface(SurfaceRaw resource) noexcept : SurfaceBase(resource)
        {
        }

        /// Copy constructor
        constexpr Surface(const Surface &other) : Surface(borrow(other.get()))
        {
        }

        /// Move constructor
        constexpr Surface(Surface &&other) noexcept : Surface(other.release())
        {
        }
         ...
         * call to [DestroySurface](https://wiki.libsdl.org/SDL3/DestroySurface) ().
         *
     ...
     * release it.
     *
     * Not all surfaces require locking. If `MustLock(surface)` evaluates to 0, then
    ...
    class SurfaceLock
    {
      ...
      public:
        ...
        /// Move constructor
        SurfaceLock(SurfaceLock &&other) noexcept : m_lock(std::move(other.m_lock))
        {
        }
    ...
    inline Surface::Surface(const PointRaw &size, PixelFormat format) : Surface(SDL_CreateSurface(size, format))
    {
    }

    inline Surface::Surface(const PointRaw &size, PixelFormat format, void *pixels, int pitch)
        : Surface(SDL_CreateSurfaceFrom(size, format, pixels, pitch))
    {
    }
     ...
     * @sa DestroySurface
     */
    inline Surface CreateSurfaceFrom(const PointRaw &size, PixelFormat format, void *pixels, int pitch)
    {
     ...
     * @sa CreateSurfaceFrom
     */
    inline void DestroySurface(SurfaceRaw surface)
    {
        SDL_DestroySurface(surface);
    }

    inline void SurfaceBase::Destroy()
    {
        DestroySurface(release());
    }
    ...
    namespace prop::Surface
    {
        ...
        constexpr auto SDR_WHITE_POINT_FLOAT = SDL_PROP_SURFACE_SDR_WHITE_POINT_FLOAT; ///< Float for sdr white point.

        constexpr auto HDR_HEADROOM_FLOAT = SDL_PROP_SURFACE_HDR_HEADROOM_FLOAT; ///< Float for hdr headroom.
        ...
        constexpr auto HOTSPOT_X_NUMBER = SDL_PROP_SURFACE_HOTSPOT_X_NUMBER; ///< Number for hotspot x.

        constexpr auto HOTSPOT_Y_NUMBER = SDL_PROP_SURFACE_HOTSPOT_Y_NUMBER; ///< Number for hotspot y.

#endif // SDL_VERSION_ATLEAST(3, 2, 6)

#if SDL_VERSION_ATLEAST(3, 4, 0)

        constexpr auto ROTATION_FLOAT = SDL_PROP_SURFACE_ROTATION_FLOAT; ///< Float for rotation.
    ...
    inline SurfaceLock SurfaceBase::Lock()
    {
        return {SurfaceRef(*this)};
    }

    inline SurfaceLock::SurfaceLock(SurfaceRef resource) : m_lock(std::move(resource))
    {
        LockSurface(m_lock);
    }
     ...
     * @sa LockSurface
     */
    inline void UnlockSurface(SurfaceRef surface)
    {
     ...
     *
     * The new surface should be freed with DestroySurface(). Not doing so will
     ...
     * @sa SaveBMP
     */
    inline Surface LoadBMP(StringParam file)
    {
     ...
     * @sa SaveBMP
     */
    inline void SaveBMP_IO(SurfaceConstRef surface, IOStreamRef dst, bool closeio = false)
    {
     ...
     * @sa SaveTrustedPNG
     */
    inline void SaveTrustedPNG_IO(SurfaceConstRef surface, IOStreamRef dst, bool closeio = false)
    {
    ...
    inline bool SurfaceBase::HasRLE() const
    {
        return SDL::SurfaceHasRLE(get());
    }
    ...
    inline void SurfaceBase::ClearColorKey()
    {
        SDL::ClearSurfaceColorKey(get());
    }
     ...
     * @sa SetSurfaceColorMod
     */
    inline void GetSurfaceColorMod(SurfaceConstRef surface, Uint8 *r, Uint8 *g, Uint8 *b)
    {
    ...
    inline void SurfaceBase::SetMod(Color color)
    {
        SDL::SetSurfaceMod(get(), color);
    }
    ...
    inline Color SurfaceBase::GetMod() const
    {
        return SDL::GetSurfaceMod(get());
    }
     ...
     * @sa GetSurfaceClipRect
     */
    inline bool SetSurfaceClipRect(SurfaceRef surface, OptionalRef<const RectRaw> rect)
    {
    ...
    inline void SurfaceBase::ResetClipRect()
    {
        SDL::ResetSurfaceClipRect(get());
    }
    ...
    inline void SurfaceBase::Flip(FlipMode flip)
    {
        SDL::FlipSurface(get(), flip);
    }
     ...
     * @sa DestroySurface
     */
    inline Surface ScaleSurface(SurfaceConstRef surface, const PointRaw &size, ScaleMode scaleMode)
    {
        return SDL_ScaleSurface(surface, size, scaleMode);
    }

    inline Surface SurfaceBase::Scale(const PointRaw &size, ScaleMode scaleMode) const
    {
    ...
    {
        return SDL::ConvertSurfaceAndColorspace(get(), format, palette, colorspace, props);
    }
    ...
    inline void ConvertPixels(const PointRaw &size,
                              ...
                              int dst_pitch)
    {
        CheckError(SDL_ConvertPixels(size, src_format, src, src_pitch, dst_format, dst, dst_pitch));
    }
    ...
    inline void PremultiplyAlpha(const PointRaw &size,
                                 ...
                                 bool linear)
    {
        CheckError(SDL_PremultiplyAlpha(size, src_format, src, src_pitch, dst_format, dst, dst_pitch, linear));
    }
    ...
    inline void SurfaceBase::Clear(const FColorRaw &c)
    {
        SDL::ClearSurface(get(), c);
    }
     ...
     *
     * If there is a clip rectangle set on the destination (set via
     * SetSurfaceClipRect()), then this function will fill based on the intersection
     ...
     * @sa FillSurfaceRects
     */
    inline void FillSurfaceRect(SurfaceRef dst, OptionalRef<const RectRaw> rect, Uint32 color)
    {
    ...
    inline void SurfaceBase::Fill(Uint32 color)
    {
        SDL::FillSurface(get(), color);
    }
     ...
     * @sa FillSurfaceRect
     */
    inline void FillSurfaceRects(SurfaceRef dst, SpanRef<const RectRaw> rects, Uint32 color)
    {
    ...
    inline void SurfaceBase::Blit(SurfaceRef src,
                                  ...
                                  OptionalRef<const RectRaw> dstrect)
    {
        SDL::BlitSurface(get(), src, srcrect, dstrect);
    }

    inline void SurfaceBase::BlitAt(SurfaceRef src, OptionalRef<const RectRaw> srcrect, const PointRaw &dstpos)
    {
        static_assert(false, "Not implemented");
    }
     ...
     * @sa BlitSurface
     */
    inline void BlitSurfaceUnchecked(SurfaceRef src, const RectRaw &srcrect, SurfaceRef dst, const RectRaw &dstrect)
    {
        CheckError(SDL_BlitSurfaceUnchecked(src, srcrect, dst, dstrect));
    }

    inline void SurfaceBase::BlitUnchecked(SurfaceRef src, const RectRaw &srcrect, const RectRaw &dstrect)
    {
        SDL::BlitSurfaceUnchecked(get(), src, srcrect, dstrect);
    }
    ...
    inline void SurfaceBase::BlitScaled(SurfaceRef src,
                                        ...
                                        ScaleMode scaleMode)
    {
        SDL::BlitSurfaceScaled(get(), src, srcrect, dstrect, scaleMode);
    }
    ...
    inline void BlitSurfaceUncheckedScaled(SurfaceRef src,
                                           ...
                                           ScaleMode scaleMode)
    {
        CheckError(SDL_BlitSurfaceUncheckedScaled(src, srcrect, dst, dstrect, scaleMode));
    }
    ...
    inline void SurfaceBase::BlitUncheckedScaled(SurfaceRef src,
                                                 ...
                                                 ScaleMode scaleMode)
    {
        SDL::BlitSurfaceUncheckedScaled(get(), src, srcrect, dstrect, scaleMode);
    }
    ...
    inline void SurfaceBase::Stretch(SurfaceRef src,
                                     ...
                                     ScaleMode scaleMode)
    {
        SDL::StretchSurface(get(), src, srcrect, dstrect, scaleMode);
    }
    ...
    inline void SurfaceBase::BlitTiled(SurfaceRef src,
                                       ...
                                       OptionalRef<const RectRaw> dstrect)
    {
        SDL::BlitSurfaceTiled(get(), src, srcrect, dstrect);
    }
    ...
    inline void BlitSurfaceTiledWithScale(SurfaceRef src,
                                          ...
                                          OptionalRef<const RectRaw> dstrect)
    {
        CheckError(SDL_BlitSurfaceTiledWithScale(src, srcrect, scale, scaleMode, dst, dstrect));
    }
    ...
    inline void SurfaceBase::BlitTiledWithScale(SurfaceRef src,
                                                ...
                                                OptionalRef<const RectRaw> dstrect)
    {
        SDL::BlitSurfaceTiledWithScale(get(), src, srcrect, scale, scaleMode, dstrect);
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void ReadSurfacePixel(SurfaceConstRef surface, const PointRaw &p, Uint8 *r, Uint8 *g, Uint8 *b, Uint8 *a)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void ReadSurfacePixel(const SurfaceLock &lock, const PointRaw &p, Uint8 *r, Uint8 *g, Uint8 *b, Uint8 *a)
    {
    ...
    inline void SurfaceBase::ReadPixel(const PointRaw &p, Uint8 *r, Uint8 *g, Uint8 *b, Uint8 *a) const
    {
    ...
    inline void SurfaceBase::ReadPixelFloat(const PointRaw &p, float *r, float *g, float *b, float *a) const
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void WriteSurfacePixelFloat(SurfaceRef surface, const PointRaw &p, const FColorRaw &c)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void WriteSurfacePixelFloat(SurfaceLock &lock, const PointRaw &p, const FColorRaw &c)
    {
    ...
    constexpr int SurfaceBase::GetWidth() const
    {
        return SDL::GetSurfaceWidth(get());
    }
    ...
    constexpr int SurfaceBase::GetHeight() const
    {
    ...
    constexpr Point SurfaceBase::GetSize() const
    {
        return SDL::GetSurfaceSize(get());
    }
    ...
    constexpr int SurfaceBase::GetPitch() const
    {
        return SDL::GetSurfacePitch(get());
    }
