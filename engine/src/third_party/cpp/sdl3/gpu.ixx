...
namespace SDL
{
     ...
     * window with ClaimWindowForGPUDevice()--although strictly speaking you can
     ...
     * long as they are submitted in the correct order, but many apps will just need
     ...
     *
     * The app calls BeginGPURenderPass(). Then it sets states it needs for each
     * draw:
     *
     ...
     *
     * The API also has "compute" support. The app calls BeginGPUComputePass() with
     ...
     * UnmapGPUTransferBuffer(), and then UploadToGPUTexture(). The next time you
     * write texture data to the transfer buffer, if you set the cycle param to
     * true, you don't have to worry about overwriting any data that is not yet
     * uploaded.
     *
    ...
    class GPUBuffer
    {
      ...
      public:
         ...
         */
        constexpr GPUBuffer(GPUBufferRaw gPUBuffer = {}) noexcept : m_gPUBuffer(gPUBuffer)
        {
        }
     ...
     * @sa GenerateMipmapsForGPUTexture
     * @sa BlitGPUTexture
    ...
    class GPUTexture
    {
      ...
      public:
         ...
         */
        constexpr GPUTexture(GPUTextureRaw gPUTexture = {}) noexcept : m_gPUTexture(gPUTexture)
        {
        }
         ...
         * texture, either via UploadToGPUTexture or by performing a render or compute
         * pass with this texture as a target.
         *
         * Note that certain combinations of usage flags are invalid. For example, a
         ...
         * @sa UploadToGPUTexture
         * @sa DownloadFromGPUTexture
         * @sa BeginGPURenderPass
         * @sa BeginGPUComputePass
         * @sa BindGPUVertexSamplers
         * @sa BindGPUVertexStorageTextures
         * @sa BindGPUFragmentSamplers
         * @sa BindGPUFragmentStorageTextures
    ...
    class GPUSampler
    {
      ...
      public:
         ...
         */
        constexpr GPUSampler(GPUSamplerRaw gPUSampler = {}) noexcept : m_gPUSampler(gPUSampler)
        {
        }
    ...
    class GPUShader
    {
      ...
      public:
         ...
         */
        constexpr GPUShader(GPUShaderRaw gPUShader = {}) noexcept : m_gPUShader(gPUShader)
        {
        }
    ...
    class GPURenderPass
    {
      ...
      public:
         ...
         */
        constexpr GPURenderPass(GPURenderPassRaw gPURenderPass = {}) noexcept : m_gPURenderPass(gPURenderPass)
        {
        }
         ...
         * @sa CreateGPUShader
         */
        void BindFragmentSamplers(Uint32 first_slot,
    ...
    class GPUComputePass
    {
      ...
      public:
         ...
         */
        constexpr GPUComputePass(GPUComputePassRaw gPUComputePass = {}) noexcept : m_gPUComputePass(gPUComputePass)
        {
        }
     ...
     * height of passed GPUTextureRegion to UploadToGPUTexture or
     * DownloadFromGPUTexture are used as default values respectively and data is
    ...
    class GPUCopyPass
    {
      ...
      public:
         ...
         */
        constexpr GPUCopyPass(GPUCopyPassRaw gPUCopyPass = {}) noexcept : m_gPUCopyPass(gPUCopyPass)
        {
        }
    ...
    class GPUCommandBuffer
    {
      ...
      public:
         ...
         *
         * On Direct3D 12, using InsertGPUDebugLabel requires WinPixEventRuntime.dll
         ...
         * PopGPUDebugGroup.
         *
         * On Direct3D 12, using PushGPUDebugGroup requires WinPixEventRuntime.dll to
         * be in your PATH or in the same directory as your executable. See
         * [here](https://devblogs.microsoft.com/pix/winpixeventruntime/) for
         * instructions on how to obtain it.
         *
         ...
         *
         * On Direct3D 12, using PopGPUDebugGroup requires WinPixEventRuntime.dll to
         * be in your PATH or in the same directory as your executable. See
         * [here](https://devblogs.microsoft.com/pix/winpixeventruntime/) for
         * instructions on how to obtain it.
         *
         ...
         * @sa EndGPURenderPass
         */
        GPURenderPass BeginRenderPass(std::span<const GPUColorTargetInfo> color_target_infos,
         ...
         * @sa EndGPUComputePass
         */
        GPUComputePass BeginComputePass(std::span<const GPUStorageTextureReadWriteBinding> storage_texture_bindings,
         ...
         * WaitAndAcquireGPUSwapchainTexture() unless you know what you are doing with
         * timing.
         *
         ...
         * @sa SubmitGPUCommandBuffer
         * @sa SubmitGPUCommandBufferAndAcquireFence
         * @sa CancelGPUCommandBuffer
         * @sa GetWindowSizeInPixels
         * @sa WaitForGPUSwapchain
         * @sa WaitAndAcquireGPUSwapchainTexture
         * @sa SetGPUAllowedFramesInFlight
         */
        GPUTexture AcquireSwapchainTexture(WindowRef window,
         ...
         * automatically be submitted for presentation when the command buffer is
         * submitted. The swapchain texture should only be referenced by the command
         * buffer used to acquire it. It is an error to call CancelGPUCommandBuffer()
         ...
         */
        GPUTexture WaitAndAcquireSwapchainTexture(WindowRef window,
                                                  Uint32 *swapchain_texture_width = nullptr,
                                                  Uint32 *swapchain_texture_height = nullptr);
    ...
    constexpr GPUShaderFormat GPU_SHADERFORMAT_INVALID = SDL_GPU_SHADERFORMAT_INVALID; ///< INVALID

    constexpr GPUShaderFormat GPU_SHADERFORMAT_PRIVATE = SDL_GPU_SHADERFORMAT_PRIVATE; ///< Shaders for NDA'd platforms.

    constexpr GPUShaderFormat GPU_SHADERFORMAT_SPIRV = SDL_GPU_SHADERFORMAT_SPIRV; ///< SPIR-V shaders for Vulkan.

    constexpr GPUShaderFormat GPU_SHADERFORMAT_DXBC = SDL_GPU_SHADERFORMAT_DXBC; ///< DXBC SM5_1 shaders for D3D12.

    constexpr GPUShaderFormat GPU_SHADERFORMAT_DXIL = SDL_GPU_SHADERFORMAT_DXIL; ///< DXIL SM6_0 shaders for D3D12.

    constexpr GPUShaderFormat GPU_SHADERFORMAT_MSL = SDL_GPU_SHADERFORMAT_MSL; ///< MSL shaders for Metal.
     ...
     * certain systems.
     *
     * It is recommended to query WindowSupportsGPUSwapchainComposition after
     ...
     *
     * It is recommended to query WindowSupportsGPUPresentMode after claiming the
     * window if you wish to change the present mode to IMMEDIATE or MAILBOX.
     *
    ...
    constexpr GPUPresentMode GPU_PRESENTMODE_VSYNC = SDL_GPU_PRESENTMODE_VSYNC; ///< GPU_PRESENTMODE_VSYNC

    constexpr GPUPresentMode GPU_PRESENTMODE_IMMEDIATE = SDL_GPU_PRESENTMODE_IMMEDIATE; ///< GPU_PRESENTMODE_IMMEDIATE

    constexpr GPUPresentMode GPU_PRESENTMODE_MAILBOX = SDL_GPU_PRESENTMODE_MAILBOX; ///< GPU_PRESENTMODE_MAILBOX

    /**
     * Specifies the pixel format of a texture.
     *
     * Texture format support varies depending on driver, hardware, and usage flags.
     * In general, you should use GPUTextureSupportsFormat to query if a format is
     * supported before using it. However, there are a few guaranteed formats.
     *
    ...
    constexpr GPUTextureFormat GPU_TEXTUREFORMAT_INVALID = SDL_GPU_TEXTUREFORMAT_INVALID; ///< GPU_TEXTUREFORMAT_INVALID
    ...
    constexpr GPUTextureFormat GPU_TEXTUREFORMAT_R8_UINT = SDL_GPU_TEXTUREFORMAT_R8_UINT; ///< GPU_TEXTUREFORMAT_R8_UINT
    ...
    constexpr GPUTextureFormat GPU_TEXTUREFORMAT_R8_INT = SDL_GPU_TEXTUREFORMAT_R8_INT; ///< GPU_TEXTUREFORMAT_R8_INT
    ...
    constexpr GPUTextureFormat GPU_TEXTUREFORMAT_R16_INT = SDL_GPU_TEXTUREFORMAT_R16_INT; ///< GPU_TEXTUREFORMAT_R16_INT
    ...
    constexpr GPUTextureFormat GPU_TEXTUREFORMAT_R32_INT = SDL_GPU_TEXTUREFORMAT_R32_INT; ///< GPU_TEXTUREFORMAT_R32_INT
    ...
    constexpr GPUTextureType GPU_TEXTURETYPE_2D = SDL_GPU_TEXTURETYPE_2D; ///< The texture is a 2-dimensional image.

    constexpr GPUTextureType GPU_TEXTURETYPE_2D_ARRAY =
        SDL_GPU_TEXTURETYPE_2D_ARRAY; ///< The texture is a 2-dimensional array image.

    constexpr GPUTextureType GPU_TEXTURETYPE_3D = SDL_GPU_TEXTURETYPE_3D; ///< The texture is a 3-dimensional image.

    constexpr GPUTextureType GPU_TEXTURETYPE_CUBE = SDL_GPU_TEXTURETYPE_CUBE; ///< The texture is a cube image.
    ...
    constexpr GPUTextureUsageFlags GPU_TEXTUREUSAGE_COMPUTE_STORAGE_READ = SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_READ;

    /// Texture supports storage writes in the compute stage.
    constexpr GPUTextureUsageFlags GPU_TEXTUREUSAGE_COMPUTE_STORAGE_WRITE = SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_WRITE;
    ...
    constexpr GPUSampleCount GPU_SAMPLECOUNT_1 = SDL_GPU_SAMPLECOUNT_1; ///< No multisampling.
    ...
    struct GPUDeviceBase : ResourceBaseT<GPUDeviceRaw>
    {
         ...
         * in an unspecified format. If prop.GPUDevice.DRIVER_INFO_STRING is available
         ...
         *
         * This must be called before AcquireGPUSwapchainTexture is called using the
         * window. You should only call this function from the thread that created the
         * window.
         *
         ...
         * you must call SetGPUSwapchainParameters after claiming the window.
         *
         * @param window an Window.
         * @throws Error on failure.
         *
         ...
         * working on the first frame, AcquireGPUSwapchainTexture() will fill the
         * swapchain texture pointer with nullptr, and
         * WaitAndAcquireGPUSwapchainTexture() will block.
         *
    ...
    struct GPUDevice : GPUDeviceBase
    {
         ...
         */
        constexpr explicit GPUDevice(GPUDeviceRaw resource) noexcept : GPUDeviceBase(resource)
        {
        }

        /// Move constructor
        constexpr GPUDevice(GPUDevice &&other) noexcept : GPUDevice(other.release())
        {
        }
     ...
     * written to memory.
     */
    constexpr GPUStoreOp GPU_STOREOP_RESOLVE_AND_STORE = SDL_GPU_STOREOP_RESOLVE_AND_STORE;
    ...
    constexpr GPUCubeMapFace GPU_CUBEMAPFACE_POSITIVEX = SDL_GPU_CUBEMAPFACE_POSITIVEX; ///< GPU_CUBEMAPFACE_POSITIVEX

    constexpr GPUCubeMapFace GPU_CUBEMAPFACE_NEGATIVEX = SDL_GPU_CUBEMAPFACE_NEGATIVEX; ///< GPU_CUBEMAPFACE_NEGATIVEX

    constexpr GPUCubeMapFace GPU_CUBEMAPFACE_POSITIVEY = SDL_GPU_CUBEMAPFACE_POSITIVEY; ///< GPU_CUBEMAPFACE_POSITIVEY

    constexpr GPUCubeMapFace GPU_CUBEMAPFACE_NEGATIVEY = SDL_GPU_CUBEMAPFACE_NEGATIVEY; ///< GPU_CUBEMAPFACE_NEGATIVEY

    constexpr GPUCubeMapFace GPU_CUBEMAPFACE_POSITIVEZ = SDL_GPU_CUBEMAPFACE_POSITIVEZ; ///< GPU_CUBEMAPFACE_POSITIVEZ

    constexpr GPUCubeMapFace GPU_CUBEMAPFACE_NEGATIVEZ = SDL_GPU_CUBEMAPFACE_NEGATIVEZ; ///< GPU_CUBEMAPFACE_NEGATIVEZ
    ...
    constexpr GPUBufferUsageFlags GPU_BUFFERUSAGE_VERTEX = SDL_GPU_BUFFERUSAGE_VERTEX; ///< Buffer is a vertex buffer.

    constexpr GPUBufferUsageFlags GPU_BUFFERUSAGE_INDEX = SDL_GPU_BUFFERUSAGE_INDEX; ///< Buffer is an index buffer.
    ...
    constexpr GPUBufferUsageFlags GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ = SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ;

    /// Buffer supports storage writes in the compute stage.
    constexpr GPUBufferUsageFlags GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE = SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE;
    ...
    constexpr GPUShaderStage GPU_SHADERSTAGE_VERTEX = SDL_GPU_SHADERSTAGE_VERTEX; ///< GPU_SHADERSTAGE_VERTEX

    constexpr GPUShaderStage GPU_SHADERSTAGE_FRAGMENT = SDL_GPU_SHADERSTAGE_FRAGMENT; ///< GPU_SHADERSTAGE_FRAGMENT
    ...
    constexpr GPUVertexInputRate GPU_VERTEXINPUTRATE_VERTEX = SDL_GPU_VERTEXINPUTRATE_VERTEX;

    /// Attribute addressing is a function of the instance index.
    constexpr GPUVertexInputRate GPU_VERTEXINPUTRATE_INSTANCE = SDL_GPU_VERTEXINPUTRATE_INSTANCE;
    ...
    constexpr GPUFillMode GPU_FILLMODE_FILL = SDL_GPU_FILLMODE_FILL; ///< Polygons will be rendered via rasterization.

    constexpr GPUFillMode GPU_FILLMODE_LINE = SDL_GPU_FILLMODE_LINE; ///< Polygon edges will be drawn as line segments.
    ...
    constexpr GPUCullMode GPU_CULLMODE_NONE = SDL_GPU_CULLMODE_NONE; ///< No triangles are culled.

    constexpr GPUCullMode GPU_CULLMODE_FRONT = SDL_GPU_CULLMODE_FRONT; ///< Front-facing triangles are culled.

    constexpr GPUCullMode GPU_CULLMODE_BACK = SDL_GPU_CULLMODE_BACK; ///< Back-facing triangles are culled.
     ...
     * front-facing.
     */
    constexpr GPUFrontFace GPU_FRONTFACE_COUNTER_CLOCKWISE = SDL_GPU_FRONTFACE_COUNTER_CLOCKWISE;
    ...
    constexpr GPUCompareOp GPU_COMPAREOP_INVALID = SDL_GPU_COMPAREOP_INVALID; ///< GPU_COMPAREOP_INVALID

    constexpr GPUCompareOp GPU_COMPAREOP_NEVER = SDL_GPU_COMPAREOP_NEVER; ///< The comparison always evaluates false.

    constexpr GPUCompareOp GPU_COMPAREOP_LESS = SDL_GPU_COMPAREOP_LESS; ///< The comparison evaluates reference < test.
    ...
    constexpr GPUCompareOp GPU_COMPAREOP_ALWAYS = SDL_GPU_COMPAREOP_ALWAYS; ///< The comparison always evaluates true.
    ...
    constexpr GPUStencilOp GPU_STENCILOP_INVALID = SDL_GPU_STENCILOP_INVALID; ///< GPU_STENCILOP_INVALID

    constexpr GPUStencilOp GPU_STENCILOP_KEEP = SDL_GPU_STENCILOP_KEEP; ///< Keeps the current value.

    constexpr GPUStencilOp GPU_STENCILOP_ZERO = SDL_GPU_STENCILOP_ZERO; ///< Sets the value to 0.

    constexpr GPUStencilOp GPU_STENCILOP_REPLACE = SDL_GPU_STENCILOP_REPLACE; ///< Sets the value to reference.

    /// Increments the current value and clamps to the maximum value.
    constexpr GPUStencilOp GPU_STENCILOP_INCREMENT_AND_CLAMP = SDL_GPU_STENCILOP_INCREMENT_AND_CLAMP;
    ...
    constexpr GPUStencilOp GPU_STENCILOP_INVERT = SDL_GPU_STENCILOP_INVERT; ///< Bitwise-inverts the current value.
    ...
    constexpr GPUStencilOp GPU_STENCILOP_DECREMENT_AND_WRAP = SDL_GPU_STENCILOP_DECREMENT_AND_WRAP;
    ...
    constexpr GPUBlendOp GPU_BLENDOP_INVALID = SDL_GPU_BLENDOP_INVALID; ///< GPU_BLENDOP_INVALID
    ...
    constexpr GPUBlendOp GPU_BLENDOP_REVERSE_SUBTRACT = SDL_GPU_BLENDOP_REVERSE_SUBTRACT;

    constexpr GPUBlendOp GPU_BLENDOP_MIN = SDL_GPU_BLENDOP_MIN; ///< min(source, destination)

    constexpr GPUBlendOp GPU_BLENDOP_MAX = SDL_GPU_BLENDOP_MAX; ///< max(source, destination)
    ...
    constexpr GPUBlendFactor GPU_BLENDFACTOR_INVALID = SDL_GPU_BLENDFACTOR_INVALID; ///< GPU_BLENDFACTOR_INVALID
    ...
    constexpr GPUBlendFactor GPU_BLENDFACTOR_SRC_COLOR = SDL_GPU_BLENDFACTOR_SRC_COLOR; ///< source color

    constexpr GPUBlendFactor GPU_BLENDFACTOR_ONE_MINUS_SRC_COLOR =
        SDL_GPU_BLENDFACTOR_ONE_MINUS_SRC_COLOR; ///< 1 - source color

    constexpr GPUBlendFactor GPU_BLENDFACTOR_DST_COLOR = SDL_GPU_BLENDFACTOR_DST_COLOR; ///< destination color

    constexpr GPUBlendFactor GPU_BLENDFACTOR_ONE_MINUS_DST_COLOR =
        SDL_GPU_BLENDFACTOR_ONE_MINUS_DST_COLOR; ///< 1 - destination color

    constexpr GPUBlendFactor GPU_BLENDFACTOR_SRC_ALPHA = SDL_GPU_BLENDFACTOR_SRC_ALPHA; ///< source alpha

    constexpr GPUBlendFactor GPU_BLENDFACTOR_ONE_MINUS_SRC_ALPHA =
        SDL_GPU_BLENDFACTOR_ONE_MINUS_SRC_ALPHA; ///< 1 - source alpha

    constexpr GPUBlendFactor GPU_BLENDFACTOR_DST_ALPHA = SDL_GPU_BLENDFACTOR_DST_ALPHA; ///< destination alpha

    constexpr GPUBlendFactor GPU_BLENDFACTOR_ONE_MINUS_DST_ALPHA =
        SDL_GPU_BLENDFACTOR_ONE_MINUS_DST_ALPHA; ///< 1 - destination alpha

    constexpr GPUBlendFactor GPU_BLENDFACTOR_CONSTANT_COLOR = SDL_GPU_BLENDFACTOR_CONSTANT_COLOR; ///< blend constant
    ...
    constexpr GPUColorComponentFlags GPU_COLORCOMPONENT_R = SDL_GPU_COLORCOMPONENT_R; ///< the red component

    constexpr GPUColorComponentFlags GPU_COLORCOMPONENT_G = SDL_GPU_COLORCOMPONENT_G; ///< the green component

    constexpr GPUColorComponentFlags GPU_COLORCOMPONENT_B = SDL_GPU_COLORCOMPONENT_B; ///< the blue component

    constexpr GPUColorComponentFlags GPU_COLORCOMPONENT_A = SDL_GPU_COLORCOMPONENT_A; ///< the alpha component
    ...
    constexpr GPUFilter GPU_FILTER_NEAREST = SDL_GPU_FILTER_NEAREST; ///< Point filtering.

    constexpr GPUFilter GPU_FILTER_LINEAR = SDL_GPU_FILTER_LINEAR; ///< Linear filtering.
    ...
    constexpr GPUSamplerAddressMode GPU_SAMPLERADDRESSMODE_MIRRORED_REPEAT = SDL_GPU_SAMPLERADDRESSMODE_MIRRORED_REPEAT;

    /// Specifies that the coordinates will clamp to the 0-1 range.
    constexpr GPUSamplerAddressMode GPU_SAMPLERADDRESSMODE_CLAMP_TO_EDGE = SDL_GPU_SAMPLERADDRESSMODE_CLAMP_TO_EDGE;
     ...
     * pipeline.
     *
     * When you call BindGPUVertexBuffers, you specify the binding slots of the
     * vertex buffers. For example if you called BindGPUVertexBuffers with a
     * first_slot of 2 and num_bindings of 3, the binding slots 2, 3, 4 would be
     ...
     * @sa CreateGPUDevice
     */
    inline bool GPUSupportsShaderFormats(GPUShaderFormat format_flags, StringParam name)
    {
     ...
     * @sa GPUSupportsShaderFormats
     */
    inline GPUDevice CreateGPUDevice(GPUShaderFormat format_flags, bool debug_mode, StringParam name)
    {
        return GPUDevice(format_flags, debug_mode, std::move(name));
    }

    inline GPUDevice::GPUDevice(GPUShaderFormat format_flags, bool debug_mode, StringParam name)
        : GPUDevice(CheckError(SDL_CreateGPUDevice(format_flags, debug_mode, name)))
    {
    }

    inline GPUDevice::GPUDevice(PropertiesRef props) : GPUDevice(CheckError(SDL_CreateGPUDeviceWithProperties(props)))
    {
    }
    ...
    namespace prop::GPUDevice::Create
    {
        ...
        constexpr auto DEBUGMODE_BOOLEAN = SDL_PROP_GPU_DEVICE_CREATE_DEBUGMODE_BOOLEAN; ///< Enable debugmode.
        ...
        constexpr auto VERBOSE_BOOLEAN = SDL_PROP_GPU_DEVICE_CREATE_VERBOSE_BOOLEAN; ///< Enable verbose.

#endif // SDL_VERSION_ATLEAST(3, 4, 0)

        constexpr auto NAME_STRING = SDL_PROP_GPU_DEVICE_CREATE_NAME_STRING; ///< String for name.

#if SDL_VERSION_ATLEAST(3, 4, 0)

        constexpr auto FEATURE_CLIP_DISTANCE_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_FEATURE_CLIP_DISTANCE_BOOLEAN; ///< Enable feature clip distance.

        constexpr auto FEATURE_DEPTH_CLAMPING_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_FEATURE_DEPTH_CLAMPING_BOOLEAN; ///< Enable feature depth clamping.

        constexpr auto FEATURE_INDIRECT_DRAW_FIRST_INSTANCE_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_FEATURE_INDIRECT_DRAW_FIRST_INSTANCE_BOOLEAN; ///< Enable feature indirect draw
                                                                                     ///< first instance.

        constexpr auto FEATURE_ANISOTROPY_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_FEATURE_ANISOTROPY_BOOLEAN; ///< Enable feature anisotropy.

#endif // SDL_VERSION_ATLEAST(3, 4, 0)

        constexpr auto SHADERS_PRIVATE_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_SHADERS_PRIVATE_BOOLEAN; ///< Enable shaders private.

        constexpr auto SHADERS_SPIRV_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_SHADERS_SPIRV_BOOLEAN; ///< Enable shaders spirv.

        constexpr auto SHADERS_DXBC_BOOLEAN = SDL_PROP_GPU_DEVICE_CREATE_SHADERS_DXBC_BOOLEAN; ///< Enable shaders dxbc.

        constexpr auto SHADERS_DXIL_BOOLEAN = SDL_PROP_GPU_DEVICE_CREATE_SHADERS_DXIL_BOOLEAN; ///< Enable shaders dxil.

        constexpr auto SHADERS_MSL_BOOLEAN = SDL_PROP_GPU_DEVICE_CREATE_SHADERS_MSL_BOOLEAN; ///< Enable shaders msl.

        constexpr auto SHADERS_METALLIB_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_SHADERS_METALLIB_BOOLEAN; ///< Enable shaders metallib.

#if SDL_VERSION_ATLEAST(3, 4, 0)

        constexpr auto D3D12_ALLOW_FEWER_RESOURCE_SLOTS_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_D3D12_ALLOW_FEWER_RESOURCE_SLOTS_BOOLEAN; ///< Enable d3d12 allow fewer resource
                                                                                 ///< slots.

#endif // SDL_VERSION_ATLEAST(3, 4, 0)

        constexpr auto D3D12_SEMANTIC_NAME_STRING =
            SDL_PROP_GPU_DEVICE_CREATE_D3D12_SEMANTIC_NAME_STRING; ///< String for d3d12 semantic name.

#if SDL_VERSION_ATLEAST(3, 4, 2)

        constexpr auto D3D12_AGILITY_SDK_VERSION_NUMBER =
            SDL_PROP_GPU_DEVICE_CREATE_D3D12_AGILITY_SDK_VERSION_NUMBER; ///< Number for d3d12 agility sdk version.

        constexpr auto D3D12_AGILITY_SDK_PATH_STRING =
            SDL_PROP_GPU_DEVICE_CREATE_D3D12_AGILITY_SDK_PATH_STRING; ///< String for d3d12 agility sdk path.
        ...
        constexpr auto VULKAN_REQUIRE_HARDWARE_ACCELERATION_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_VULKAN_REQUIRE_HARDWARE_ACCELERATION_BOOLEAN; ///< Enable vulkan require hardware
                                                                                     ///< acceleration.

        constexpr auto VULKAN_OPTIONS_POINTER =
            SDL_PROP_GPU_DEVICE_CREATE_VULKAN_OPTIONS_POINTER; ///< Pointer to vulkan options.
        ...
        constexpr auto METAL_ALLOW_MACFAMILY1_BOOLEAN =
            SDL_PROP_GPU_DEVICE_CREATE_METAL_ALLOW_MACFAMILY1_BOOLEAN; ///< Enable metal allow macfamily1.
    ...
    inline void GPUDeviceBase::Destroy()
    {
        DestroyGPUDevice(release());
    }
     ...
     * @sa GetNumGPUDrivers
     */
    inline const char *GetGPUDriver(int index)
    {
     ...
     * - venus
     *
     * `prop.GPUDevice.DRIVER_VERSION_STRING`: Contains the self-reported version of
     * the underlying system driver. This is a relatively short version string in an
     * unspecified format. If prop.GPUDevice.DRIVER_INFO_STRING is available then
     * that property should be preferred over this one as it may contain additional
    ...
    namespace prop::GPUDevice
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_GPU_DEVICE_NAME_STRING; ///< String for name.

        constexpr auto DRIVER_NAME_STRING = SDL_PROP_GPU_DEVICE_DRIVER_NAME_STRING; ///< String for driver name.

        constexpr auto DRIVER_VERSION_STRING =
            SDL_PROP_GPU_DEVICE_DRIVER_VERSION_STRING; ///< String for driver version.

        constexpr auto DRIVER_INFO_STRING = SDL_PROP_GPU_DEVICE_DRIVER_INFO_STRING; ///< String for driver info.
     ...
     * @sa ReleaseGPUComputePipeline
     */
    inline GPUComputePipeline CreateGPUComputePipeline(GPUDeviceRef device,
                                                       const GPUComputePipelineCreateInfo &createinfo)
    {
        return GPUComputePipeline(device, createinfo);
    }

    inline GPUComputePipeline GPUDeviceBase::CreateComputePipeline(const GPUComputePipelineCreateInfo &createinfo)
    {
        return GPUComputePipeline(get(), createinfo);
    }

    inline GPUComputePipeline::GPUComputePipeline(GPUDeviceRef device, const GPUComputePipelineCreateInfo &createinfo)
        : m_gPUComputePipeline(CheckError(SDL_CreateGPUComputePipeline(device, &createinfo)))
    {
    }
    ...
    namespace prop::GPUComputePipeline::Create
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_GPU_COMPUTEPIPELINE_CREATE_NAME_STRING; ///< String for name.
     ...
     * @sa ReleaseGPUGraphicsPipeline
     */
    inline GPUGraphicsPipeline CreateGPUGraphicsPipeline(GPUDeviceRef device,
                                                         const GPUGraphicsPipelineCreateInfo &createinfo)
    {
        return GPUGraphicsPipeline(device, createinfo);
    }

    inline GPUGraphicsPipeline GPUDeviceBase::CreateGraphicsPipeline(const GPUGraphicsPipelineCreateInfo &createinfo)
    {
        return GPUGraphicsPipeline(get(), createinfo);
    }

    inline GPUGraphicsPipeline::GPUGraphicsPipeline(GPUDeviceRef device,
                                                    const GPUGraphicsPipelineCreateInfo &createinfo)
        : m_gPUGraphicsPipeline(CheckError(SDL_CreateGPUGraphicsPipeline(device, &createinfo)))
    {
    }
    ...
    namespace prop::GPUGraphicsPipeline::Create
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_GPU_GRAPHICSPIPELINE_CREATE_NAME_STRING; ///< String for name.
     ...
     * @sa ReleaseGPUSampler
     */
    inline GPUSampler CreateGPUSampler(GPUDeviceRef device, const GPUSamplerCreateInfo &createinfo)
    {
        return GPUSampler(device, createinfo);
    }

    inline GPUSampler GPUDeviceBase::CreateSampler(const GPUSamplerCreateInfo &createinfo)
    {
        return GPUSampler(get(), createinfo);
    }

    inline GPUSampler::GPUSampler(GPUDeviceRef device, const GPUSamplerCreateInfo &createinfo)
    ...
    namespace prop::GPUSampler::Create
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_GPU_SAMPLER_CREATE_NAME_STRING; ///< String for name.
     ...
     * @sa ReleaseGPUShader
     */
    inline GPUShader CreateGPUShader(GPUDeviceRef device, const GPUShaderCreateInfo &createinfo)
    {
        return GPUShader(device, createinfo);
    }

    inline GPUShader GPUDeviceBase::CreateShader(const GPUShaderCreateInfo &createinfo)
    {
        return GPUShader(get(), createinfo);
    }

    inline GPUShader::GPUShader(GPUDeviceRef device, const GPUShaderCreateInfo &createinfo)
    ...
    namespace prop::GPUShader::Create
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_GPU_SHADER_CREATE_NAME_STRING; ///< String for name.
     ...
     * @sa GPUTextureSupportsFormat
     */
    inline GPUTexture CreateGPUTexture(GPUDeviceRef device, const GPUTextureCreateInfo &createinfo)
    {
        return GPUTexture(device, createinfo);
    }

    inline GPUTexture GPUDeviceBase::CreateTexture(const GPUTextureCreateInfo &createinfo)
    {
        return GPUTexture(get(), createinfo);
    }

    inline GPUTexture::GPUTexture(GPUDeviceRef device, const GPUTextureCreateInfo &createinfo)
    ...
    namespace prop::GPUTexture::Create
    {
        ...
        constexpr auto D3D12_CLEAR_DEPTH_FLOAT =
            SDL_PROP_GPU_TEXTURE_CREATE_D3D12_CLEAR_DEPTH_FLOAT; ///< Float for d3d12 clear depth.

#if SDL_VERSION_ATLEAST(3, 2, 12)

        constexpr auto D3D12_CLEAR_STENCIL_NUMBER =
            SDL_PROP_GPU_TEXTURE_CREATE_D3D12_CLEAR_STENCIL_NUMBER; ///< Number for d3d12 clear stencil.

#endif // SDL_VERSION_ATLEAST(3, 2, 12)

        constexpr auto NAME_STRING = SDL_PROP_GPU_TEXTURE_CREATE_NAME_STRING; ///< String for name.
     ...
     * @sa ReleaseGPUBuffer
     */
    inline GPUBuffer CreateGPUBuffer(GPUDeviceRef device, const GPUBufferCreateInfo &createinfo)
    {
        return GPUBuffer(device, createinfo);
    }

    inline GPUBuffer GPUDeviceBase::CreateBuffer(const GPUBufferCreateInfo &createinfo)
    {
        return GPUBuffer(get(), createinfo);
    }

    inline GPUBuffer::GPUBuffer(GPUDeviceRef device, const GPUBufferCreateInfo &createinfo)
    ...
    namespace prop::GPUBuffer::Create
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_GPU_BUFFER_CREATE_NAME_STRING; ///< String for name.
     ...
     * @sa ReleaseGPUTransferBuffer
     */
    inline GPUTransferBuffer CreateGPUTransferBuffer(GPUDeviceRef device, const GPUTransferBufferCreateInfo &createinfo)
    {
        return GPUTransferBuffer(device, createinfo);
    }

    inline GPUTransferBuffer GPUDeviceBase::CreateTransferBuffer(const GPUTransferBufferCreateInfo &createinfo)
    {
        return GPUTransferBuffer(get(), createinfo);
    }

    inline GPUTransferBuffer::GPUTransferBuffer(GPUDeviceRef device, const GPUTransferBufferCreateInfo &createinfo)
        : m_gPUTransferBuffer(CheckError(SDL_CreateGPUTransferBuffer(device, &createinfo)))
    {
    }
    ...
    namespace prop::GPUTransferBuffer::Create
    {
        ...
        constexpr auto NAME_STRING = SDL_PROP_GPU_TRANSFERBUFFER_CREATE_NAME_STRING; ///< String for name.
     ...
     * @sa CreateGPUBuffer
     */
    inline void SetGPUBufferName(GPUDeviceRef device, GPUBuffer buffer, StringParam text)
    {
     ...
     * @sa CreateGPUTexture
     */
    inline void SetGPUTextureName(GPUDeviceRef device, GPUTexture texture, StringParam text)
    {
     ...
     * Useful for debugging.
     *
     * On Direct3D 12, using InsertGPUDebugLabel requires WinPixEventRuntime.dll to
     * be in your PATH or in the same directory as your executable. See
     * [here](https://devblogs.microsoft.com/pix/winpixeventruntime/) for
     * instructions on how to obtain it.
     *
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void InsertGPUDebugLabel(GPUCommandBuffer command_buffer, StringParam text)
    {
     ...
     * PopGPUDebugGroup.
     *
     * On Direct3D 12, using PushGPUDebugGroup requires WinPixEventRuntime.dll to be
     * in your PATH or in the same directory as your executable. See
     * [here](https://devblogs.microsoft.com/pix/winpixeventruntime/) for
     * instructions on how to obtain it.
     *
     * On some backends (e.g. Metal), pushing a debug group during a
     ...
     * Ends the most-recently pushed debug group.
     *
     * On Direct3D 12, using PopGPUDebugGroup requires WinPixEventRuntime.dll to be
     * in your PATH or in the same directory as your executable. See
     * [here](https://devblogs.microsoft.com/pix/winpixeventruntime/) for
     * instructions on how to obtain it.
     *
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void ReleaseGPUTransferBuffer(GPUDeviceRef device, GPUTransferBuffer transfer_buffer)
    {
        SDL_ReleaseGPUTransferBuffer(device, transfer_buffer);
    }

    inline void GPUDeviceBase::ReleaseTransferBuffer(GPUTransferBuffer transfer_buffer)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void ReleaseGPUComputePipeline(GPUDeviceRef device, GPUComputePipeline compute_pipeline)
    {
        SDL_ReleaseGPUComputePipeline(device, compute_pipeline);
    }

    inline void GPUDeviceBase::ReleaseComputePipeline(GPUComputePipeline compute_pipeline)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void ReleaseGPUGraphicsPipeline(GPUDeviceRef device, GPUGraphicsPipeline graphics_pipeline)
    {
        SDL_ReleaseGPUGraphicsPipeline(device, graphics_pipeline);
    }

    inline void GPUDeviceBase::ReleaseGraphicsPipeline(GPUGraphicsPipeline graphics_pipeline)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void PushGPUVertexUniformData(GPUCommandBuffer command_buffer, Uint32 slot_index, SourceBytes data)
    {
        SDL_PushGPUVertexUniformData(command_buffer, slot_index, data.data(), data.size_bytes());
    }

    inline void GPUCommandBuffer::PushVertexUniformData(Uint32 slot_index, SourceBytes data)
    {
        SDL::PushGPUVertexUniformData(m_gPUCommandBuffer, slot_index, std::move(data));
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void PushGPUFragmentUniformData(GPUCommandBuffer command_buffer, Uint32 slot_index, SourceBytes data)
    {
        SDL_PushGPUFragmentUniformData(command_buffer, slot_index, data.data(), data.size_bytes());
    }

    inline void GPUCommandBuffer::PushFragmentUniformData(Uint32 slot_index, SourceBytes data)
    {
        SDL::PushGPUFragmentUniformData(m_gPUCommandBuffer, slot_index, std::move(data));
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void PushGPUComputeUniformData(GPUCommandBuffer command_buffer, Uint32 slot_index, SourceBytes data)
    {
        SDL_PushGPUComputeUniformData(command_buffer, slot_index, data.data(), data.size_bytes());
    }

    inline void GPUCommandBuffer::PushComputeUniformData(Uint32 slot_index, SourceBytes data)
    {
        SDL::PushGPUComputeUniformData(m_gPUCommandBuffer, slot_index, std::move(data));
    }
     ...
     * @sa EndGPURenderPass
     */
    inline GPURenderPass BeginGPURenderPass(GPUCommandBuffer command_buffer,
                                            std::span<const GPUColorTargetInfo> color_target_infos,
                                            OptionalRef<const GPUDepthStencilTargetInfo> depth_stencil_target_info)
    {
        return SDL_BeginGPURenderPass(command_buffer, color_target_infos, depth_stencil_target_info);
    }
    ...
    inline GPURenderPass GPUCommandBuffer::BeginRenderPass(
        ...
        OptionalRef<const GPUDepthStencilTargetInfo> depth_stencil_target_info)
    {
        return SDL::BeginGPURenderPass(m_gPUCommandBuffer, color_target_infos, depth_stencil_target_info);
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void BindGPUGraphicsPipeline(GPURenderPass render_pass, GPUGraphicsPipeline graphics_pipeline)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void SetGPUViewport(GPURenderPass render_pass, const GPUViewport &viewport)
    {
     ...
     * @sa GPU_BLENDFACTOR_ONE_MINUS_CONSTANT_COLOR
     */
    inline void SetGPUBlendConstants(GPURenderPass render_pass, FColorRaw blend_constants)
    {
    ...
    inline void GPURenderPass::BindVertexBuffers(Uint32 first_slot, std::span<const GPUBufferBinding> bindings)
    {
    ...
    inline void GPURenderPass::BindIndexBuffer(const GPUBufferBinding &binding, GPUIndexElementSize index_element_size)
    {
     ...
     * @sa CreateGPUShader
     */
    inline void BindGPUVertexSamplers(GPURenderPass render_pass,
                                      Uint32 first_slot,
                                      std::span<const GPUTextureSamplerBinding> texture_sampler_bindings)
    {
    ...
    inline void GPURenderPass::BindVertexSamplers(Uint32 first_slot,
                                                  std::span<const GPUTextureSamplerBinding> texture_sampler_bindings)
    {
        SDL::BindGPUVertexSamplers(m_gPURenderPass, first_slot, texture_sampler_bindings);
    }
     ...
     * @sa CreateGPUShader
     */
    inline void BindGPUVertexStorageTextures(GPURenderPass render_pass,
                                             Uint32 first_slot,
    ...
    inline void GPURenderPass::BindVertexStorageTextures(Uint32 first_slot,
                                                         SpanRef<const GPUTextureRaw> storage_textures)
    {
        SDL::BindGPUVertexStorageTextures(m_gPURenderPass, first_slot, storage_textures);
    }
    ...
    inline void GPURenderPass::BindVertexStorageBuffers(Uint32 first_slot, SpanRef<const GPUBufferRaw> storage_buffers)
    {
        SDL::BindGPUVertexStorageBuffers(m_gPURenderPass, first_slot, storage_buffers);
    }
     ...
     * @sa CreateGPUShader
     */
    inline void BindGPUFragmentSamplers(GPURenderPass render_pass,
                                        Uint32 first_slot,
                                        std::span<const GPUTextureSamplerBinding> texture_sampler_bindings)
    {
        SDL_BindGPUFragmentSamplers(render_pass, first_slot, texture_sampler_bindings);
    }

    inline void GPURenderPass::BindFragmentSamplers(Uint32 first_slot,
                                                    std::span<const GPUTextureSamplerBinding> texture_sampler_bindings)
    {
        SDL::BindGPUFragmentSamplers(m_gPURenderPass, first_slot, texture_sampler_bindings);
    }
     ...
     * @sa CreateGPUShader
     */
    inline void BindGPUFragmentStorageTextures(GPURenderPass render_pass,
                                               Uint32 first_slot,
                                               SpanRef<const GPUTextureRaw> storage_textures)
    {
    ...
    inline void GPURenderPass::BindFragmentStorageTextures(Uint32 first_slot,
                                                           SpanRef<const GPUTextureRaw> storage_textures)
    {
        SDL::BindGPUFragmentStorageTextures(m_gPURenderPass, first_slot, storage_textures);
    }
    ...
    inline void GPURenderPass::BindFragmentStorageBuffers(Uint32 first_slot,
                                                          SpanRef<const GPUBufferRaw> storage_buffers)
    {
        SDL::BindGPUFragmentStorageBuffers(m_gPURenderPass, first_slot, storage_buffers);
    }
    ...
    inline void GPURenderPass::DrawPrimitives(Uint32 num_vertices,
                                              ...
                                              Uint32 first_instance)
    {
        SDL::DrawGPUPrimitives(m_gPURenderPass, num_vertices, num_instances, first_vertex, first_instance);
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void DrawGPUPrimitivesIndirect(GPURenderPass render_pass, GPUBuffer buffer, Uint32 offset, Uint32 draw_count)
    {
        SDL_DrawGPUPrimitivesIndirect(render_pass, buffer, offset, draw_count);
    }

    inline void GPURenderPass::DrawPrimitivesIndirect(GPUBuffer buffer, Uint32 offset, Uint32 draw_count)
    {
    ...
    inline void GPURenderPass::DrawIndexedPrimitivesIndirect(GPUBuffer buffer, Uint32 offset, Uint32 draw_count)
    {
        SDL::DrawGPUIndexedPrimitivesIndirect(m_gPURenderPass, buffer, offset, draw_count);
    }
    ...
    inline void GPURenderPass::End()
    {
        SDL::EndGPURenderPass(m_gPURenderPass);
    }
    ...
    inline GPUComputePass BeginGPUComputePass(
        ...
        std::span<const GPUStorageBufferReadWriteBinding> storage_buffer_bindings)
    {
        return SDL_BeginGPUComputePass(command_buffer, storage_texture_bindings, storage_buffer_bindings);
    }
    ...
    inline GPUComputePass GPUCommandBuffer::BeginComputePass(
        ...
        std::span<const GPUStorageBufferReadWriteBinding> storage_buffer_bindings)
    {
        return SDL::BeginGPUComputePass(m_gPUCommandBuffer, storage_texture_bindings, storage_buffer_bindings);
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void BindGPUComputePipeline(GPUComputePass compute_pass, GPUComputePipeline compute_pipeline)
    {
     ...
     * @sa CreateGPUComputePipeline
     */
    inline void BindGPUComputeSamplers(GPUComputePass compute_pass,
                                       Uint32 first_slot,
                                       std::span<const GPUTextureSamplerBinding> texture_sampler_bindings)
    {
        SDL_BindGPUComputeSamplers(compute_pass, first_slot, texture_sampler_bindings);
    }

    inline void GPUComputePass::BindSamplers(Uint32 first_slot,
                                             std::span<const GPUTextureSamplerBinding> texture_sampler_bindings)
    {
        SDL::BindGPUComputeSamplers(m_gPUComputePass, first_slot, texture_sampler_bindings);
    }
     ...
     * @sa CreateGPUComputePipeline
     */
    inline void BindGPUComputeStorageTextures(GPUComputePass compute_pass,
                                              Uint32 first_slot,
                                              SpanRef<const GPUTextureRaw> storage_textures)
    {
    ...
    inline void GPUComputePass::BindStorageTextures(Uint32 first_slot, SpanRef<const GPUTextureRaw> storage_textures)
    {
        SDL::BindGPUComputeStorageTextures(m_gPUComputePass, first_slot, storage_textures);
    }
    ...
    inline void GPUComputePass::BindStorageBuffers(Uint32 first_slot, SpanRef<const GPUBufferRaw> storage_buffers)
    {
        SDL::BindGPUComputeStorageBuffers(m_gPUComputePass, first_slot, storage_buffers);
    }
    ...
    inline void GPUComputePass::Dispatch(Uint32 groupcount_x, Uint32 groupcount_y, Uint32 groupcount_z)
    {
        SDL::DispatchGPUCompute(m_gPUComputePass, groupcount_x, groupcount_y, groupcount_z);
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void DispatchGPUComputeIndirect(GPUComputePass compute_pass, GPUBuffer buffer, Uint32 offset)
    {
    ...
    inline void GPUComputePass::End()
    {
        SDL::EndGPUComputePass(m_gPUComputePass);
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void *MapGPUTransferBuffer(GPUDeviceRef device, GPUTransferBuffer transfer_buffer, bool cycle)
    {
        return CheckError(SDL_MapGPUTransferBuffer(device, transfer_buffer, cycle));
    }

    inline void *GPUDeviceBase::MapTransferBuffer(GPUTransferBuffer transfer_buffer, bool cycle)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void UnmapGPUTransferBuffer(GPUDeviceRef device, GPUTransferBuffer transfer_buffer)
    {
        SDL_UnmapGPUTransferBuffer(device, transfer_buffer);
    }

    inline void GPUDeviceBase::UnmapTransferBuffer(GPUTransferBuffer transfer_buffer)
    {
    ...
    inline void GPUCopyPass::CopyTextureToTexture(const GPUTextureLocation &source,
                                                  const GPUTextureLocation &destination,
                                                  Uint32 w,
                                                  Uint32 h,
                                                  Uint32 d,
                                                  bool cycle)
    {
        SDL::CopyGPUTextureToTexture(m_gPUCopyPass, source, destination, w, h, d, cycle);
    }
    ...
    inline void GPUCopyPass::CopyBufferToBuffer(const GPUBufferLocation &source,
                                                const GPUBufferLocation &destination,
                                                Uint32 size,
                                                bool cycle)
    {
    ...
    inline void GPUCopyPass::DownloadFromTexture(const GPUTextureRegion &source,
                                                 const GPUTextureTransferInfo &destination)
    {
    ...
    inline void GPUCopyPass::DownloadFromBuffer(const GPUBufferRegion &source,
                                                const GPUTransferBufferLocation &destination)
    {
    ...
    inline void GPUCopyPass::End()
    {
        SDL::EndGPUCopyPass(m_gPUCopyPass);
    }
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void GenerateMipmapsForGPUTexture(GPUCommandBuffer command_buffer, GPUTexture texture)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline void BlitGPUTexture(GPUCommandBuffer command_buffer, const GPUBlitInfo &info)
    {
     ...
     * @sa ClaimWindowForGPUDevice
     */
    inline bool WindowSupportsGPUSwapchainComposition(GPUDeviceRef device,
                                                      WindowRef window,
                                                      GPUSwapchainComposition swapchain_composition)
    {
    ...
    inline bool GPUDeviceBase::WindowSupportsSwapchainComposition(WindowRef window,
                                                                  GPUSwapchainComposition swapchain_composition)
    {
        return SDL::WindowSupportsGPUSwapchainComposition(get(), window, swapchain_composition);
    }
     ...
     * @sa ClaimWindowForGPUDevice
     */
    inline bool WindowSupportsGPUPresentMode(GPUDeviceRef device, WindowRef window, GPUPresentMode present_mode)
    {
        return SDL_WindowSupportsGPUPresentMode(device, window, present_mode);
    }

    inline bool GPUDeviceBase::WindowSupportsPresentMode(WindowRef window, GPUPresentMode present_mode)
    {
     ...
     * @sa WindowSupportsGPUSwapchainComposition
     */
    inline bool SetGPUSwapchainParameters(GPUDeviceRef device,
                                          WindowRef window,
                                          GPUSwapchainComposition swapchain_composition,
                                          GPUPresentMode present_mode)
    {
    ...
    inline bool GPUDeviceBase::SetSwapchainParameters(WindowRef window,
                                                      GPUSwapchainComposition swapchain_composition,
                                                      GPUPresentMode present_mode)
    {
        return SDL::SetGPUSwapchainParameters(get(), window, swapchain_composition, present_mode);
    }
     ...
     * on the first frame, AcquireGPUSwapchainTexture() will fill the swapchain
     * texture pointer with nullptr, and WaitAndAcquireGPUSwapchainTexture() will
     * block.
     *
     * Higher values increase throughput at the expense of visual latency. Lower
     * values decrease visual latency at the expense of throughput.
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline bool SetGPUAllowedFramesInFlight(GPUDeviceRef device, Uint32 allowed_frames_in_flight)
    {
        return SDL_SetGPUAllowedFramesInFlight(device, allowed_frames_in_flight);
    }

    inline bool GPUDeviceBase::SetAllowedFramesInFlight(Uint32 allowed_frames_in_flight)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline GPUTextureFormat GetGPUSwapchainTextureFormat(GPUDeviceRef device, WindowRef window)
    {
        return SDL_GetGPUSwapchainTextureFormat(device, window);
    }

    inline GPUTextureFormat GPUDeviceBase::GetSwapchainTextureFormat(WindowRef window)
    {
     ...
     * @param command_buffer a command buffer.
     * @param window a window that has been claimed.
     * @param swapchain_texture a pointer filled in with a swapchain texture handle.
     ...
     * @sa SetGPUAllowedFramesInFlight
     */
    inline GPUTexture AcquireGPUSwapchainTexture(GPUCommandBuffer command_buffer,
                                                 WindowRef window,
                                                 Uint32 *swapchain_texture_width = nullptr,
                                                 Uint32 *swapchain_texture_height = nullptr)
    {
        return CheckError(
            SDL_AcquireGPUSwapchainTexture(command_buffer, window, swapchain_texture_width, swapchain_texture_height));
    }

    inline GPUTexture GPUCommandBuffer::AcquireSwapchainTexture(WindowRef window,
     ...
     * @sa AcquireGPUSwapchainTexture
     */
    inline GPUTexture WaitAndAcquireGPUSwapchainTexture(GPUCommandBuffer command_buffer,
                                                        WindowRef window,
                                                        Uint32 *swapchain_texture_width = nullptr,
                                                        Uint32 *swapchain_texture_height = nullptr)
    {
        return CheckError(SDL_WaitAndAcquireGPUSwapchainTexture(command_buffer,
                                                                window,
                                                                swapchain_texture_width,
                                                                swapchain_texture_height));
    }

    inline GPUTexture GPUCommandBuffer::WaitAndAcquireSwapchainTexture(WindowRef window,
                                                                       Uint32 *swapchain_texture_width,
                                                                       Uint32 *swapchain_texture_height)
    {
     ...
     * @sa ReleaseGPUFence
     */
    inline GPUFence *SubmitGPUCommandBufferAndAcquireFence(GPUCommandBuffer command_buffer)
    {
    ...
    inline void GPUDeviceBase::WaitForIdle()
    {
        SDL::WaitForGPUIdle(get());
    }
     ...
     * @sa WaitForGPUIdle
     */
    inline void WaitForGPUFences(GPUDeviceRef device, bool wait_all, std::span<GPUFence *const> fences)
    {
        CheckError(SDL_WaitForGPUFences(device, wait_all, fences));
    }

    inline void GPUDeviceBase::WaitForFences(bool wait_all, std::span<GPUFence *const> fences)
    {
     ...
     * @since This function is available since SDL 3.2.0.
     */
    inline bool GPUTextureSupportsSampleCount(GPUDeviceRef device, GPUTextureFormat format, GPUSampleCount sample_count)
    {
        return SDL_GPUTextureSupportsSampleCount(device, format, sample_count);
    }

    inline bool GPUDeviceBase::TextureSupportsSampleCount(GPUTextureFormat format, GPUSampleCount sample_count)
    {
     ...
     * @sa AddEventWatch
     */
    inline void GDKSuspendGPU(GPUDeviceRef device)
    {
        SDL_GDKSuspendGPU(device);
    }

    inline void GPUDeviceBase::GDKSuspendGPU()
    {
        SDL::GDKSuspendGPU(get());
    }
     ...
     * @sa AddEventWatch
     */
    inline void GDKResumeGPU(GPUDeviceRef device)
    {
        SDL_GDKResumeGPU(device);
    }

    inline void GPUDeviceBase::GDKResumeGPU()
    {
        SDL::GDKResumeGPU(get());
    }
