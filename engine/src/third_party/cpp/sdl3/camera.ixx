...
namespace SDL
{
    ...
    constexpr CameraPosition CAMERA_POSITION_UNKNOWN = SDL_CAMERA_POSITION_UNKNOWN; ///< CAMERA_POSITION_UNKNOWN
    ...
    struct Camera : CameraBase
    {
         ...
         */
        constexpr explicit Camera(CameraRaw resource) noexcept : CameraBase(resource)
        {
        }

        /// Move constructor
        constexpr Camera(Camera &&other) noexcept : Camera(other.release())
        {
        }
    ...
    struct CameraFrame : Surface
    {
        ...
        /// Move constructor
        CameraFrame(CameraFrame &&other) noexcept : m_lock(std::move(other.m_lock))
        {
        }
     ...
     * @sa GetCameraFormat
     */
    inline Camera OpenCamera(CameraID instance_id, OptionalRef<const CameraSpec> spec = {})
    {
    ...
    inline CameraID CameraBase::GetID()
    {
        return SDL::GetCameraID(get());
    }
     ...
     * @sa ReleaseCameraFrame
     */
    inline Surface AcquireCameraFrame(CameraRef camera, Uint64 *timestampNS = nullptr)
    {
    ...
    inline CameraFrame::CameraFrame(CameraRef resource) : m_lock(std::move(resource))
    {
     ...
     * @sa OpenCamera
     */
    inline void CloseCamera(CameraRaw camera)
    {
        SDL_CloseCamera(camera);
    }

    inline void CameraBase::Close()
    {
        CloseCamera(release());
    }
