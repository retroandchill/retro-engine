...
#define SDL3PP_ENABLE_STRING_PARAM
    ...
    export class StringParam
    {
      ...
      public:
         ...
         */
        StringParam(const char *str = "") : data(str)
        {
        }
         ...
         *            object.
         */
        StringParam(const std::string &str) : StringParam(str.c_str())
        {
        }
         ...
         */
        StringParam(std::string &&str) : data(std::move(str))
        {
        }
         ...
         */
        StringParam(std::string_view str) : StringParam(std::string{str})
        {
        }
     ...
     * A simple std::string-like interface for SDL allocated strings.
     */
    export struct StringResult : OwnArray<char>
    {
        /// Use parent ctors
        using OwnArray::OwnArray;

        /// Copy ctor
        StringResult(const StringResult &other) : StringResult(std::string_view(other))
        {
        }

        /// Move ctor
        constexpr StringResult(StringResult &&other) noexcept : OwnArray(other.release(), other.size())
        {
        }
    ...
    export class SourceBytes
    {
      ...
      public:
        ...
        constexpr SourceBytes(std::nullptr_t) : SourceBytes()
        {
        }
        ...
        constexpr SourceBytes(std::span<T, N> span) : SourceBytes(span.data(), span.size_bytes())
        {
        }
         ...
         */
        template <class T>
        constexpr SourceBytes(const T &data) : SourceBytes(std::span{data.data(), data.size()})
        {
        }
        /**
         * From array
         ...
         */
        template <class T, size_t N>
        constexpr SourceBytes(T (&data)[N]) : SourceBytes(std::span<T, N>{static_cast<T *>(data), N})
        {
        }
        ...
        constexpr SourceBytes(const void *data, size_t size_bytes)
            : m_data(size_bytes > 0 ? data : nullptr), m_size_bytes(data != nullptr ? size_bytes : 0)
        {
        }
    ...
    export class TargetBytes
    {
      ...
      public:
        ...
        /// Disambiguate between multiple pointer types.
        constexpr TargetBytes(std::nullptr_t) : TargetBytes()
        {
        }
         ...
         */
        template <class T, size_t N>
        constexpr TargetBytes(std::span<T, N> span) : TargetBytes(span.data(), span.size_bytes())
        {
        }
         ...
         */
        template <class T>
        constexpr TargetBytes(T &data) : TargetBytes(std::span{data.data(), data.size()})
        {
        }
        /**
         * From array
         ...
         */
        template <class T, size_t N>
        constexpr TargetBytes(T (&data)[N]) : TargetBytes(std::span<T, N>{static_cast<T *>(data), N})
        {
        }
