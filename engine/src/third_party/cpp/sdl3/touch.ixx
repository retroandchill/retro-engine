...
namespace SDL
{
    ...
    constexpr TouchDeviceType TOUCH_DEVICE_INVALID = SDL_TOUCH_DEVICE_INVALID; ///< TOUCH_DEVICE_INVALID
    ...
    struct Finger : FingerRaw
    {
         ...
         */
        constexpr Finger(const FingerRaw &finger = {}) noexcept : FingerRaw(finger)
        {
        }
         ...
         */
        constexpr Finger(FingerID id, float x, float y, float pressure) noexcept : FingerRaw{id, x, y, pressure}
        {
        }
    ...
    {
        return SDL_GetTouchFingers(touchID);
    }
