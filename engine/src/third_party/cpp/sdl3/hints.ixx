 ...
 *
 * If this hint is true, the activity can be recreated on demand by the OS, and
 * Java static data and C++ static data remain with their current values. If
 * this hint is false, then SDL will call exit() when you return from your main
 * function and the application will be terminated and then started fresh each
 * time.
 *
 * The variable can be set to the following values:
 *
 ...
 * For example, this will often appear in volume control applets for audio
 * streams, and in lists of applications which are inhibiting the screensaver.
...
/**
 * A variable controlling whether controllers used with the Apple TV generate UI
 * events.
 *
 ...
 * device, this hint can be used to specify a default channel count that will be
 * used. This defaults to "1" for recording and "2" for playback devices.
 *
 * This hint should be set before an audio device is opened.
 *
 * @since This hint is available since SDL 3.2.0.
 ...
 *
 * Some audio backends (such as Pulseaudio and Pipewire) allow you to set an XDG
 * icon name for your application. Among other things, this icon might show up
 ...
 *
 * This hint is an integer > 0, that represents the size of the device's buffer
 * in sample frames (stereo audio data in 16-bit format is 4 bytes per sample
 * frame, for example).
 *
 * SDL3 generally decides this value on behalf of the app, but if for some
 * reason the app needs to dictate this (because they want either lower latency
 * or higher throughput AND ARE WILLING TO DEAL WITH what that might require of
 * the app), they can specify it.
 *
 * SDL will try to accommodate this value, but there is no promise you'll get
 * the buffer size requested. Many platforms won't honor this request at all, or
 * might adjust it.
 *
 * This hint should be set before an audio device is opened.
 *
 ...
 * control panel that lets the user adjust the volume on specific audio streams
 ...
 *
 * Note that while this talks about audio streams, this is an OS-level concept,
 * so it applies to a physical audio device in this case, and not an
 ...
 *
 * If the application doesn't specify the audio format when opening the device,
 * this hint can be used to specify a default format that will be used.
 *
 * The variable can be set to the following values:
 *
 ...
 * scenarios where you want to hook up SDL to some sort of virtual device, etc.
 *
 * The variable can be set to the following values:
 *
 ...
 * The variable can be set to the following values:
 *
 * - "0": Surfaces with a colorkey or an alpha channel are saved to a 32-bit BMP
 *   file with an alpha mask. SDL will use the bitmap header version 4 and set
 *   the alpha mask accordingly. (default)
 * - "1": Surfaces with a colorkey or an alpha channel are saved to a 32-bit BMP
 *   file without an alpha mask. The alpha channel data will be in the file, but
 *   applications are going to ignore it.
 *
 * This hint can be set anytime.
 *
 * @since This hint is available since SDL 3.2.0.
 ...
 *
 * By default, SDL will try all available camera backends in a reasonable order
 * until it finds one that can work, but this hint allows the app or user to
 * force a specific target, such as "directshow" if, say, you are on Windows
 * Media Foundations but want to try DirectShow instead.
 *
 * The default value is unset, in which case SDL will try to figure out the best
 * camera backend on your behalf. This hint needs to be set before Init() is
 * called to be useful.
 *
 * @since This hint is available since SDL 3.2.0.
 */
 ...
 *
 * By default, SDL will try all available dialog backends in a reasonable order
 * until it finds one that can work, but this hint allows the app or user to
 ...
 *
 * This hint currently only applies to platforms using the generic "Unix" dialog
 * implementation, but may be extended to more platforms in the future. Note
 * that some Unix and Unix-like platforms have their own implementation, such as
 * macOS and Haiku.
 *
 * The variable can be set to the following values:
 *
...
/**
 * A variable controlling verbosity of the logging of SDL events pushed onto the
 * internal queue.
 *
 ...
 * application developers that need better visibility into what is going on in
 * the event queue. Logged events are sent through Log(), which means by default
 * they appear on stdout on most platforms or maybe OutputDebugString() on
 * Windows, and can be funneled by the app with SetLogOutputFunction(), etc.
 *
 * This hint can be set anytime.
 *
 * @since This hint is available since SDL 3.2.0.
 ...
 * with a 3D rendering engine. This variable controls whether and how this is
 * done.
 *
 * The variable can be set to the following values:
 *
 ...
 * should be used as sensors for gamepads.
 *
 * The variable can be set to the following values:
 *
 ...
 * platforms.
 *
 * The value must be a stringified integer, for example "10" to allow for up to
 * 10 characters of text input.
 *
 * This hint is available only if SDL_GDK_TEXTINPUT defined.
 *
 * This hint should be set before calling StartTextInput()
 ...
 *
 * By default libusb will only be used for a few devices that require direct USB
 * access, and this can be controlled with SDL_HINT_HIDAPI_LIBUSB_WHITELIST.
 *
 * The variable can be set to the following values:
 *
 ...
 * until it finds one that can work, but this hint allows the app or user to
 * force a specific target, such as "direct3d12" if, say, your hardware supports
 * Vulkan but you want to try using D3D12 instead.
 *
...
/**
 * A variable to control whether hid_enumerate() enumerates all HID devices or
 * only controllers.
 *
 * The variable can be set to the following values:
 *
 ...
 * crashing on devices with bad drivers and avoiding macOS keyboard capture
 * permission prompts.
 *
 * This hint can be set anytime.
 *
 ...
 * items:
 *
 * - "none" or "0": The application can't render any IME elements, and native UI
 *   should be used. (default)
 * - "composition": The application handles EVENT_TEXT_EDITING events and can
 *   render the composition text.
 * - "candidates": The application handles EVENT_TEXT_EDITING_CANDIDATES and can
 ...
 * The variable can be set to the following values:
 *
 * - "0": Disable joystick & gamecontroller input events when the application is
 ...
 *   doesn't change the controller report mode unless the application uses them.
 *
 ...
 * factor.
 *
 * This will override SDL_HINT_JOYSTICK_GAMECUBE_DEVICES and the built in device
 * list.
 *
 * The format of the string is a comma separated list of USB VID/PID pairs in
 * hexadecimal form, e.g.
 ...
 * The variable can be set to the following values:
 *
 * - "0": Left and right Joy-Con controllers will not be combined and each will
 *   be a mini-gamepad.
 ...
 *
 * This is useful for applications that need full compatibility for things like
 * ADSR envelopes. - Stop is implemented by setting low_frequency_rumble to 0
 * and high_frequency_rumble >0 - Rumble is both at any arbitrary value -
 ...
 * Nintendo Switch Joy-Con controller is opened.
 *
 * The variable can be set to the following values:
 *
 ...
 * connected via Bluetooth should be used.
 *
 * The variable can be set to the following values:
 *
 * - "0": HIDAPI driver is not used.
...
/**
 * A variable controlling whether the HIDAPI driver for Nintendo Online classic
 * controllers should be used.
 *
 * The variable can be set to the following values:
 *
 ...
 * - "1": HIDAPI driver is used for Steam Controllers, which requires Bluetooth
 *   access and may prompt the user for permission on iOS and Android.
 *
 * This hint should be set before initializing joysticks and gamepads.
 *
 * @since This hint is available since SDL 3.2.0.
...
/**
 * A variable controlling whether the Home button LED should be turned on when a
 * Nintendo Switch Pro controller is opened.
 *
 * The variable can be set to the following values:
 *
 * - "0": Home button LED is turned off.
 ...
 * vertical mode when using the HIDAPI driver.
 *
 * The variable can be set to the following values:
 *
...
/**
 * A variable controlling whether digital hats on Linux will apply deadzones to
 * their underlying input axes or use unfiltered values.
 *
 * The variable can be set to the following values:
 *
 * - "0": Return digital hat values based on unfiltered input axis values.
 * - "1": Return digital hat values with deadzones on the input axes taken into
 *   account. (default)
 *
 * This hint should be set before a controller is opened.
 *
 ...
 *   Windows.Gaming.Input, providing better trigger axes, guide button presses,
 *   and rumble support for Xbox controllers. (default)
 *
 * This hint should be set before a gamepad is opened.
 *
 * @since This hint is available since SDL 3.2.0.
 ...
 * controllers.
 *
 * This will override SDL_HINT_JOYSTICK_THROTTLE_DEVICES and the built in device
 * list.
 *
 * The format of the string is a comma separated list of USB VID/PID pairs in
 * hexadecimal form, e.g.
 ...
 *
 * This variable is a comma separated set of options for translating keycodes in
 * events:
 *
 * - "none": Keycode options are cleared, this overrides other options.
 * - "hide_numpad": The numpad keysyms will be translated into their non-numpad
 *   versions based on the current NumLock state. For example, SDLK_KP_4 would
 *   become SDLK_4 if KMOD_NUM is set in the event modifiers, and SDLK_LEFT if
 *   it is unset.
 ...
 * functionality, where "NN" is a device index number. SDL makes a guess at the
 * best index to use (usually zero), but the app or user can set this hint to a
 * number between 0 and 99 to force selection.
 *
 * This hint should be set before SDL is initialized.
 *
 * @since This hint is available since SDL 3.2.0.
 ...
 * is still able to process input and query attributes of attached displays, but
 * it cannot change display state or draw to the screen directly.
 *
...
/**
 * A variable controlling whether the Option key on macOS should be remapped to
 * act as the Alt key.
 *
 * The variable can be set to the following values:
 *
 ...
 *
 * If this is set to a number, it represents Hz, so "60" means try to iterate 60
 * times per second. "0" means to iterate as fast as possible. Negative values
 * are illegal, but reserved, in case they are useful in a future revision of
 * SDL.
 *
 ...
 * this hint is ignored. When the hint can be used, it is allowed to be changed
 * at any time.
 *
...
/**
 * A variable controlling whether the mouse is captured while mouse buttons are
 * pressed.
 *
 * The variable can be set to the following values:
 *
 ...
 *
 * This should be an integer corresponding to the SystemCursor enum. The default
 * value is zero (SYSTEM_CURSOR_DEFAULT).
 *
 * This hint needs to be set before Init().
 *
 * @since This hint is available since SDL 3.2.0.
 ...
 *   (default)
 *
 * If not set, this is automatically enabled unless an application uses relative
 * mouse mode directly.
 *
 * This hint can be set anytime.
 *
 ...
 * OpenGL context. If this hint isn't set, SDL will choose a reasonable default.
 *
 * @since This hint is available since SDL 3.2.0.
 */
 ...
 *
 * Circumstances where this is useful include - Testing an app with a particular
 * OpenGL ES implementation, e.g ANGLE, or emulator, e.g. those from ARM,
 * Imagination or Qualcomm. - Resolving OpenGL ES function addresses at link
 * time by linking with the OpenGL ES library instead of querying them at run
 * time with GL_GetProcAddress().
 *
 ...
 *
 * By default, when using the OpenVR driver, it will search for the API library
 * in the current folder. But, if you wish to use a system API you can specify
 * that by using this hint. This should be the full or relative path to a .dll
 ...
 * devices. If a device keeps producing new events between calls to PumpEvents,
 * a poll loop will become stuck until the new events stop. This is most
 ...
 * EVENT_LOCALE_CHANGED event (but if you can change the hint, you can push your
...
/**
 * A variable that decides whether to send EVENT_QUIT when closing the last
 * window.
 *
 * The variable can be set to the following values:
 *
 ...
 *   legitimate reasons one might get an EVENT_QUIT event: choosing "Quit" from
 *   the macOS menu bar, sending a SIGINT (ctrl-c) on Unix, etc.
 * - "1": SDL will send a quit event when the last window is requesting to
 *   close. (default)
 *
 * If there is at least one active system tray icon, EVENT_QUIT will instead be
 * sent when both the last window will be closed and the last tray icon will be
 * destroyed.
 *
 * This hint can be set anytime.
 *
 ...
 * be initialized, creating a renderer will fail.
 *
 ...
 * tried in the order listed when creating a renderer until one succeeds or all
 * of them fail.
 *
 ...
 * DisableScreenSaver() is used (or the screensaver is automatically disabled).
 * The contents of this hint are used when the screensaver is disabled. You
 * should use a string that describes what your program is doing (and,
 * therefore, why the screensaver is disabled). For example, "Playing a game" or
 * "Watching a video".
 *
 * Setting this to "" or leaving it unset will have SDL use a reasonable
 ...
 * might use dbus under the hood and this can cause crashes if they continue
 * after Quit().
 *
 * The variable can be set to the following values:
 *
 ...
 *
 * By default, SDL will try all available storage backends in a reasonable order
 * until it finds one that can work, but this hint allows the app or user to
 * force a specific target, such as "pc" if, say, you are on Steam but want to
 * avoid SteamRemoteStorage for title data.
 *
 * This hint should be set before SDL is initialized.
 *
 * @since This hint is available since SDL 3.2.0.
 ...
 * recommended for cases where low latency is an important factor because we
 * save a whole frame of latency.
 *
...
/**
 * A variable that specifies the menu visibility when a window is fullscreen in
 * Spaces on macOS.
 *
 * The variable can be set to the following values:
 *
 * - "0": The menu will be hidden when the window is in a fullscreen space, and
...
/**
 * A variable controlling whether the libdecor Wayland backend is allowed to be
 * used.
 *
...
/**
 * A variable controlling how modes with a non-native aspect ratio are displayed
 * under Wayland.
 *
...
/**
 * A variable controlling whether the libdecor Wayland backend is preferred over
 * native decorations.
 *
 ...
 *
 * This is intended to allow legacy applications to be displayed without desktop
 * scaling being applied, and has issues with certain display configurations, as
 * this forces the window to behave in a way that Wayland desktops were not
 ...
 * - The window may jump in size when moving between displays of different scale
 *   factors.
 ...
 * use two different sets of binaries, those compiled by the user from source or
 * those provided by the Chrome browser. In the later case, these binaries
 ...
 * The variable can be set to the following values:
 *
 * - "0": Don't call XSelectInput(), assuming the native window code has done it
 *   already.
...
/**
 * A variable controlling whether touch should be enabled on the front panel of
 * the PlayStation Vita.
 *
 * The variable can be set to the following values:
 *
...
/**
 * A variable controlling whether OpenGL should be used instead of OpenGL ES on
 * the PlayStation Vita.
 *
 * The variable can be set to the following values:
 *
 ...
 *   invalid, not present for non-PCM formats, or if the data chunk doesn't have
 *   that many samples.
...
/**
 * A variable controlling how the size of the RIFF chunk affects the loading of
 * a WAVE file.
 *
 ...
 * default, SDL stops after 10000 chunks to prevent wasting time. Use
 * SDL_HINT_WAVE_CHUNK_LIMIT to adjust this value.
 *
 * The variable can be set to the following values:
 *
 ...
 * beep to convey that nothing happened. This is true even if the window has no
 * menu at all!
 *
 ...
 *
 * Note: This also affects keyboard events: with mnemonics enabled, when a menu
 * is opened from the keyboard, you will not receive a KEYUP event for the
 * mnemonic key, and *might* not receive one for Alt.
 *
 * The variable can be set to the following values:
 *
 ...
 * will fall back to Kernel Objects on older OS versions or if forced to by this
 * hint.
 *
 * The variable can be set to the following values:
 *
 * - "0": Use Atomics and WaitOnAddress API when available, otherwise fall back
 ...
 * - "always_ignore": Program continues on, ignoring this assertion failure for
 *   the rest of the run.
 *
...
#define SDL_HINT_PEN_TOUCH_EVENTS "SDL_PEN_TOUCH_EVENTS"
     ...
     * @sa SetHint
     */
    inline void SetHintWithPriority(StringParam name, StringParam value, HintPriority priority)
    {
     ...
     * variable that takes precedence. You can use SetHintWithPriority() to set the
     ...
     * @sa ResetHints
     */
    inline void ResetHint(StringParam name)
    {
        CheckError(SDL_ResetHint(name));
    }
     ...
     * @sa SetHintWithPriority
     */
    inline const char *GetHint(StringParam name)
    {
     ...
     * @sa AddHintCallback
     */
    using HintCallback = void(SDLCALL *)(void *userdata, const char *name, const char *oldValue, const char *newValue);
     ...
     * @sa HintCallback
     */
    using HintCB = MakeFrontCallback<void(const char *name, const char *oldValue, const char *newValue)>;
     ...
     */
    inline void AddHintCallback(StringParam name, HintCallback callback, void *userdata)
    {
     ...
     * @sa AddHintCallback
     */
    inline void RemoveHintCallback(StringParam name, HintCallback callback, void *userdata)
    {
