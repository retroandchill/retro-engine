...
{
    public static TokenResult<string> ParseQuotedString(this TokenCursor input)
    {
        ...
        try
        {
            ...
            return result.HasValue
                ? TokenResult.Success(builder.ToString(), input, result.Remainder)
                : TokenResult.CastEmpty<Unit, string>(result);
        }
        finally
        {
            builder.Dispose();
    ...
    public static TokenResult<Unit> ParseQuotedString(this TokenCursor input, scoped ref ValueStringBuilder builder)
    {
        var openQuote = input.ParseChar('"');
        if (!openQuote.HasValue)
            return TokenResult.CastEmpty<char, Unit>(openQuote);

        const string stopCharacters = "\"\n\r";
        const string stopAndEscapeCharacters = $"{stopCharacters}\\";

        var remainder = openQuote.Remainder;
        while (true)
        {
            var unescaped = input.ParseUntilCharIn(stopAndEscapeCharacters);
            builder.Append(unescaped.TokenText);
        ...
        return closeQuote.HasValue
            ? TokenResult.Success(Unit.Value, input, remainder)
            : TokenResult.CastEmpty<char, Unit>(closeQuote);
    }
}
