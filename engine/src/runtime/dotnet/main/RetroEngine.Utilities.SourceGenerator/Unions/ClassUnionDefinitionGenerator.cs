// // @file ClassUnionDefinitionGenerator.cs
// //
// // @copyright Copyright (c) 2026 Retro & Chill. All rights reserved.
// // Licensed under the MIT License. See LICENSE file in the project root for full license information.

using Microsoft.CodeAnalysis;
using RetroEngine.Utilities.SourceGenerator.Common;
using RetroEngine.Utilities.SourceGenerator.Common.CodeAnalyzing;
using RetroEngine.Utilities.SourceGenerator.Common.CodeGenerating;
using RetroEngine.Utilities.SourceGenerator.Unions.CodeAnalyzing;
using CodeAnalyzing_TypeInfo = RetroEngine.Utilities.SourceGenerator.Common.CodeAnalyzing.TypeInfo;
using TypeInfo = RetroEngine.Utilities.SourceGenerator.Common.CodeAnalyzing.TypeInfo;
using TypeKind = RetroEngine.Utilities.SourceGenerator.Common.CodeGenerating.TypeKind;

namespace RetroEngine.Utilities.SourceGenerator.Unions;

public sealed class ClassUnionDefinitionGenerator(UnionInfo union) : IUnionDefinitionGenerator
{
    private readonly Dictionary<UnionCaseInfo, UnionCaseNestedType> _unionCaseNestedTypes = GetUnionCaseNestedTypes(
        union
    );
    ...
    {
        var caseNestedTypeInfo = _unionCaseNestedTypes[unionCase].TypeInfo;
        return unionCase.Parameters.Select(x => $"{TypeInfos.Unsafe}.As<{caseNestedTypeInfo}>(this).{x.Name}");
    }
    ...
    public TypeDefinition AdjustUnionTypeDefinition(TypeDefinition typeDefinition) =>
        typeDefinition with
        {
            Constructors =
            [
                new ConstructorDefinition { Accessibility = Accessibility.Private, BodyWriter = (_, _) => { } },
            ],
            NestedTypes = [.. _unionCaseNestedTypes.Select(x => x.Value.TypeDefinition)],
        };
    ...
    private static UnionCaseNestedType GetUnionCaseNestedType(UnionCaseInfo unionCase, UnionInfo union)
    {
        ...
        var caseTypeInfo = TypeInfo.SpecificType(
            union.TypeInfo.Namespace,
            union.TypeInfo,
            caseClassName,
            TypeInfo.TypeKind.ReferenceType(TypeInfo.ReferenceTypeKind.Class)
        );
    ...
    private static MethodDefinition[] GetUnionCaseEqualsMethods(
        UnionCaseInfo unionCase,
        CodeAnalyzing_TypeInfo caseTypeInfo,
        UnionInfo union
    )
    {
            ...
            BodyWriter = (_, methodBlock) =>
            {
                ...
                methodBlock.AppendLine(
                    UnionGenerationUtils.GetUnionCaseHashCodeCode(
                        caseIndex,
                        unionCase.Parameters.Select(x => (x.TypeName, x.Name))
                    )
                );
            },
        };
                    ...
                    methodBlock
                        ...
                        .Append(
                            UnionGenerationUtils.GetUnionCaseEqualityCode(
                                unionCase.Parameters.Select(x =>
                                    (x.TypeName, x.Name, $"{def.Parameters[0].Name}.{x.Name}")
                                )
                            )
                        )
                        .AppendLine(";");
        ...
        static void WriteEqualsMethodBody(CodeAnalyzing_TypeInfo caseTypeInfo, CodeWriter methodBodyBlock)
        {
        ...
        new()
        {
            ...
            BodyWriter = (_, methodBlock) =>
                methodBlock
                    .Append("return ")
                    .Append(
                        UnionGenerationUtils.GetCaseStringRepresentation(
                            unionCase.Name,
                            unionCase.Parameters.Select(x => (x.Name, x.Name)).ToArray()
                        )
                    )
                    .AppendLine(";"),
        };

    private readonly record struct UnionCaseNestedType(CodeAnalyzing_TypeInfo TypeInfo, TypeDefinition TypeDefinition);
}
