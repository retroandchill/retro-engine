...
using RetroEngine.Utilities.SourceGenerator.Common.Extensions;
using RetroEngine.Utilities.SourceGenerator.Unions.CodeAnalyzing;
using RetroEngine.Utilities.SourceGenerator.Unions.Extensions;
...
public sealed class StructUnionDefinitionGenerator : IUnionDefinitionGenerator
{
    ...
    public StructUnionDefinitionGenerator(UnionInfo union)
    {
        ...
        _unionImplementationGenerator = new UnionImplementationGenerator(
            Common.CodeAnalyzing.TypeInfo.SpecificType(
                union.TypeInfo.Namespace,
                union.TypeInfo.ContainingType,
                $"{union.Name}BlittableData",
                Common.CodeAnalyzing.TypeInfo.TypeKind.ValueType(true, false)
            )
        );
    ...
    private sealed class UnionImplementationGenerator(CodeAnalyzing_TypeInfo unionBlittableDataTypeInfo)
    {
                    ...
                    if (isUnmanaged && !caseParameter.ContainsGenericParameters)
                    {
                        if (!_blittableParameters.TryGetValue(unionCase, out var blittableStructInfo))
                        {
                            var typeInfo = TypeInfo.SpecificType(
                                ...
                                CodeAnalyzing_TypeInfo.TypeKind.ValueType(true, false)
                            );
        ...
        private readonly record struct CaseBlittableStructInfo(
            CodeAnalyzing_TypeInfo StructInfo,
            string FieldName,
