...
public static class TypeCodeWriter
{
    ...
    private static void WriteTypeDeclaration(TypeDefinition typeDefinition, CodeWriter codeWriter)
    {
        ...
        typeDefinition
            .Kind.Match(
                declarationBuilder,
                static (declarationBuilder, isAbstract, isSealed) =>
                    declarationBuilder.AddIf(isAbstract, () => "abstract").AddIf(isSealed, () => "sealed"),
                static (declarationBuilder, isReadOnly) => declarationBuilder.AddIf(isReadOnly, () => "readonly")
            )
        ...
        declarationBuilder
            ...
            .AddIf(
                typeDefinition.InheritedTypes.Length > 0,
    ...
    private static void WriteProperty(PropertyDefinition propertyDefinition, CodeWriter typeBodyBlock)
    {
        ...
        using (var propertyBodyBlock = typeBodyBlock.NewBlock())
        {
            ...
            if (propertyDefinition.Setter is not null)
            {
                WritePropertyAccessor(
                    ...
                    propertyDefinition.IsInitOnly ? "init" : "set",
                    propertyBodyBlock,
                    propertyDefinition
                );
            }
        ...
        static void WritePropertyAccessor(
            PropertyDefinition.PropertyAccessor propertyAccessor,
            string accessorName,
            CodeWriter propertyBodyBlock,
    ...
    private static void WriteMethodDeclaration(MethodDefinition methodDefinition, CodeWriter typeBodyBlock)
    {
        ...
        var genericParametersStr =
            methodDefinition.GenericParameters.Length > 0
                ? $"<{string.Join(", ", methodDefinition.GenericParameters.Select(x => x.Name))}>"
                : string.Empty;
        ...
        var genericParameterConstraints =
            methodDefinition.GenericParameters.Length > 0
                ? string.Join(
                    "",
                    methodDefinition
                        .GenericParameters.Where(p => p.Constraints.Length > 0)
