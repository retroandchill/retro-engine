/**
 * @file optional_monadic_test.cpp
 *
...
constexpr retro::Optional<int> get_opt_int(int)
{
    return 42;
}
...
TEST(OptionalMonadicTest, Transform)
{
    ...
    const retro::Optional<int> o5 = 40;
...
{
    // test each overload in turn
    constexpr retro::Optional<int> o16 = 42;
    constexpr auto o16r = o16.transform(get_int);
    static_assert(*o16r == 42);

    constexpr retro::Optional<int> o20 = 42;
...
TEST(OptionalMonadicTest, and_then)
{
    ...
    // ensure that function object is forwarded
    retro::Optional<int> o5 = 42;
    auto o5r = o5.and_then(rval_call_and_then{});
    static_assert((std::is_same<decltype(o5r), retro::Optional<double>>::value));
    EXPECT_TRUE(o5r.value() == 42);

    // ensure that lhs is forwarded
    retro::Optional<int> o6 = 42;
    auto o6r = std::move(o6).and_then([](int &&i) { return retro::Optional<double>(i); });
    ...
    const retro::Optional<int> o7 = 42;
...
{
    constexpr retro::Optional<int> o10 = 42;
    constexpr auto o10r = o10.and_then(get_opt_int);
    EXPECT_TRUE(*o10r == 42);

    constexpr retro::Optional<int> o11 = 42;
...
TEST(OptionalMonadicTest, or_else)
{
    ...
    EXPECT_TRUE(*(o1.or_else([] { return retro::make_optional(13); })) == 42);

    retro::Optional<int> o2;
    EXPECT_EQ(*(o2.or_else([] { return retro::make_optional(13); })), 13);

    /*
      optional<T> or_else(F&& f) && {
    */
    EXPECT_TRUE(*(std::move(o1).or_else([] { return retro::make_optional(13); })) == 42);
    EXPECT_EQ(*(std::move(o2).or_else([] { return retro::make_optional(13); })), 13);
}

TEST(OptionalMonadicTest, Constexpr_or_else)
{
    constexpr retro::Optional<int> o1 = 42;
    constexpr auto test = (*(o1.or_else([] { return retro::make_optional(13); })) == 42);
    EXPECT_TRUE(test);
    constexpr retro::Optional<int> o2;
    constexpr auto test2 = *(o2.or_else([] { return retro::make_optional(13); })) == 13;
    EXPECT_TRUE(test2);

    /*
      optional<T> or_else(F&& f) && {
    */
    constexpr auto test3 = (*(std::move(o1).or_else([] { return retro::make_optional(13); })) == 42);
    EXPECT_TRUE(test3);
    constexpr auto test4 = *(std::move(o2).or_else([] { return retro::make_optional(13); })) == 13;
    EXPECT_TRUE(test4);
}
...
constexpr bool optional_or_else_do_not_dereference_impl()
{
    ...
    int *ptr = new int(42);
    retro::Optional<int &> iref = *ptr;
    delete ptr;

    if (!iref)
        return false;

    const auto or_else_fun = []()
    {
        return retro::Optional<int &>();
    };

    // This must not dereference the pointer inside `iref`
    retro::Optional<int &> iref2 = iref.or_else(or_else_fun);
    if (!iref2)
        return false;
