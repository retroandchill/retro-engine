...
void main() {
    // Basic 2D transformation (Scale -> Rotate -> Translate)
    float s = sin(uData.rotation);
    float c = cos(uData.rotation);
    mat2 rotationMatrix = mat2(c, -s, s, c);

    vec2 transformedPos = (rotationMatrix * (inPosition * uData.scale)) + uData.position;
